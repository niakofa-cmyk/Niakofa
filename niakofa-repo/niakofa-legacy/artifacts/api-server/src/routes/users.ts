...
// POST /users/:id/sms-opt-in — collect phone during onboarding and send welcome SMS
router.post("/users/:id/sms-opt-in", requireAuth, resolveMeParam, requireOwnership(), authLimiter, async (req, res) => {
  const userId = parseInt(String(req.params.id));
  if (isNaN(userId)) return res.status(400).json({ error: "Invalid id" });
  const { phone } = req.body as { phone?: string };
  if (!phone || !/^\+?[1-9]\d{6,14}$/.test(phone.trim())) {
    return res.status(400).json({ error: "A valid phone number is required (E.164 format, e.g. +15551234567)." });
  }

  const masked = phone.trim().replace(/\d(?=\d{4})/g, "*");
  await db.update(usersTable).set({ phone_masked: masked }).where(eq(usersTable.id, userId));

  import("../lib/sms.js").then(({ sendSms }) => {
    sendSms(phone.trim(), `Welcome to Niakofa! You'll receive SMS alerts when someone needs help nearby. Reply STOP to opt out.`)
      .catch(err => logger.warn({ err, userId }, "sms-opt-in: welcome SMS failed — non-critical"));
  }).catch(err => logger.warn({ err }, "sms module import failed — non-critical"));

  logger.info({ userId }, "sms-opt-in: phone collected, welcome SMS queued");
  return res.json({ ok: true, phone_masked: masked });
});

// GET /users/:id/reliability — helper reliability score from actual request history
router.get("/users/:id/reliability", requireAuth, resolveMeParam, async (req, res) => {
  const userId = parseInt(String(req.params.id));
  if (isNaN(userId)) return res.status(400).json({ error: "Invalid id" });
  const { computeReliabilityScore } = await import("../lib/trust-tiers.js");
  const metrics = await computeReliabilityScore(userId);
