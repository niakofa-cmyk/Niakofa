...
import { adminLimiter, generalApiLimiter, paymentLimiter } from "../middlewares/rate-limit";
...
// POST /civic/needs/:id/invoice/:invoiceId/pay — sponsor pays a NET30 invoice
// online via Stripe PaymentIntent. Creates a PI for the invoice amount, returns
// the client_secret for the frontend Stripe Elements flow. The webhook handler
// in stripe.ts flips the invoice to "paid" on payment_intent.succeeded.
router.post("/civic/needs/:id/invoice/:invoiceId/pay", requireAuth, paymentLimiter, async (req, res) => {
  const needId = parseInt(req.params.id as string, 10);
  const invoiceId = parseInt(req.params.invoiceId as string, 10);
  if (isNaN(needId) || isNaN(invoiceId)) return res.status(400).json({ error: "Invalid id" });
  ...
  if (!STRIPE_SECRET_KEY) {
    return res.status(503).json({
      error: "Stripe is not configured. Contact an admin to pay by check.",
      setup: "Set STRIPE_SECRET_KEY environment variable to enable online invoice payment.",
    });
  }
  const stripe = new Stripe(STRIPE_SECRET_KEY, { apiVersion: "2024-06-20" as Stripe.LatestApiVersion });

  const [invoice] = await db.select().from(civicInvoicesTable)
    .where(and(
      eq(civicInvoicesTable.id, invoiceId),
      eq(civicInvoicesTable.civic_need_id, needId),
      eq(civicInvoicesTable.status, "pending"),
  ...
  if (amountCents < 50) return res.status(400).json({ error: "Invoice amount too small for card payment." });
  ...
  try {
    const pi = await stripe.paymentIntents.create({
      ...
      currency: "usd",
      metadata: {
        civic_invoice_id: String(invoice.id),
        civic_need_id: String(needId),
        user_id: String(req.authenticatedUserId),
      },
      description: `NET30 invoice #${invoice.id} for civic need #${needId}`,
    });

    logger.info({ invoice_id: invoice.id, need_id: needId, pi_id: pi.id, amount: amountCents }, "civic-invoice: Stripe PaymentIntent created");
    ...
    logger.error({ err, invoice_id: invoiceId }, "civic-invoice: Stripe PaymentIntent creation failed");
    return res.status(502).json({ error: "Failed to create payment intent. Please try again or pay by check." });
  }
});
