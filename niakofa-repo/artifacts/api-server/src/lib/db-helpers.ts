 ...
 *
 *   // Kill-switch flags (like nia_enabled) must be fail-closed: no defaultValue
 *   // is passed here, and the caller checks `=== "true"` explicitly so a
 *   // missing row never accidentally enables the feature.
 *   const niaEnabled = (await getSystemSetting("nia_enabled")) === "true";
 */
 ...
 *
 * @example
 *   // Fail-closed pattern for kill-switch style flags: require an explicit
 *   // "true", never treat "unset" or "false" as enabled.
 *   const niaEnabled = (await getSystemSetting("nia_enabled")) === "true";
 ...
 *   const poolEnabled = settings["pool_enabled"] !== "false"; // pool defaults ON — not a kill-switch flag
 *   // For a fail-closed kill-switch flag (e.g. nia_enabled), require explicit "true" instead:
 *   //   const niaEnabled = settings["nia_enabled"] === "true";
 */
