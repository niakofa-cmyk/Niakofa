...
async function runGeneralCheckin(): Promise<void> {
  ...
  while (true) {
    // LEFT JOIN through the user's community to their diaspora hub so this
    // worker can see hub.is_crisis — previously Nia's check-in cycle only
    // knew about county-level crisis_state and had no visibility into the
    // diaspora-hub crisis flag, so a hub-in-crisis resident could still get
    // a generic "hope everything went smoothly 💙" nudge instead of crisis
    // framing. hub.is_crisis is surfaced here; hub_in_crisis users get their
    // check-in skipped for this cycle so the dedicated crisis-followup-worker
    // (which already reads is_crisis-flagged nia_conversations) owns that
    // relationship instead of two workers messaging the same person.
    const result = await pool.query(
      `SELECT
         hr.id           AS request_id,
       ...
       LEFT JOIN diaspora_hubs dh ON dh.community_id = u.community_id AND dh.is_crisis = TRUE
    ...
    for (const req of requests) {
      try {
        ...
        //    for gentler, more frequent follow-up.
        const firstName = req.user_name?.split(" ")[0] ?? "friend";
        const niaMessage = req.hub_in_crisis
          ? [
              `Hey ${firstName} 💙`,
              ``,
              `I know things are hard for the community right now. Yesterday you received help with "${req.request_title}" — I hope that made things a little easier.`,
              ``,
              `I'm here if you need anything — local resources, someone to talk to, or help finding support nearby.`,
            ].join("\n")
          : [
              `Hey ${firstName} 💙`,
              ``,
        ...
        try {
          await pool.query(
            ...
            [
              ...
              req.hub_in_crisis
                ? `Hey ${firstName}, thinking of you and the community. Open Nia if you need anything.`
                : `Hey ${firstName}! I just wanted to check in. Open Nia to chat.`,
              JSON.stringify({ type: "nia_checkin", request_id: req.request_id, hub_in_crisis: req.hub_in_crisis }),
            ]
          );
        } catch (pushErr) {
          logger.warn(
