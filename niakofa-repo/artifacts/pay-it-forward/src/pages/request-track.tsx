...
export default function RequesterTrackingScreen() {
  ...
  // eventually catches it — wasteful and confusing on a no-token deploy.
  const [mapError, setMapError] = useState(() => !import.meta.env.VITE_MAPBOX_TOKEN);
        ...
        >
          ...
          <Marker longitude={request.lng} latitude={request.lat} anchor="center">
            <div className="relative flex items-center justify-center w-10 h-10">
              {suppressed
                ? <div className="absolute w-10 h-10 bg-primary rounded-full opacity-10" />
      ...
      {!hasHelper && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 pointer-events-none">
          <div className="bg-card/90 backdrop-blur-md border border-border rounded-2xl px-6 py-4 text-center shadow-2xl">
            <div className={`w-2 h-2 rounded-full bg-primary mx-auto mb-2${suppressed ? "" : " animate-pulse"}`} />
            <p className="font-bold text-sm">Finding a helper nearby…</p>
            <p className="text-xs text-muted-foreground mt-1">
              {nearbyHelpers.length > 0
                ? `${nearbyHelpers.length} helper${nearbyHelpers.length === 1 ? "" : "s"} online near you`
