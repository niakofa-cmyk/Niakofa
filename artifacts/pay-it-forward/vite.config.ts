...
export default defineConfig({
  ...
  server: {
    ...
    host: "0.0.0.0",
    allowedHosts: true,
    // NOTE: Vite HMR WebSocket does not connect through Replit's preview proxy —
    // the browser logs a "WebSocket closed without opened" warning but this is
    // cosmetic only. Do NOT set hmr:false here; that breaks @vitejs/plugin-react's
    // Fast Refresh preamble injection and causes a runtime crash. The warning is
    // accepted as a known Replit proxy limitation (manual refresh picks up changes).
    fs: {
      strict: true,
    ...
    // now-invalid /node_modules/.vite/deps chunk hashes ("Outdated Optimize
    // Dep" 504s). Force no-store for HTML/module requests in dev so every
    // preview load gets the current dep graph. Never runs in production.
    headers:
      process.env.NODE_ENV !== "production"
