...
function StatCard({ value, label, icon: Icon, onClick, tone }: {
  ...
  return (
    <button
      onClick={onClick}
      className={`${diasporaTheme.radius} group border border-white/10 bg-white/[0.035] p-3 text-left ${diasporaTheme.focus}`}
    >
  ...
  if (!currentUser) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-background px-6">
        <p className="text-sm text-muted-foreground">Sign in to access your Diaspora Dashboard.</p>
      </div>
    );
  }
        ...
        <div className="relative mx-auto max-w-3xl px-4 pb-6 pt-8 sm:px-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-xl text-sm leading-relaxed text-teal-50/60">
                {locale ? `From ${locale} to every place your family calls home.` : "Connect family, culture, memory, and community across generations."}
              </p>
            </div>
            <div className="hidden h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-amber-300/30 bg-amber-300/10 sm:flex">
              <BookHeart className="h-7 w-7 text-amber-300" />
            </div>
          </div>
        ...
        <div className="mx-auto flex max-w-3xl gap-1 overflow-x-auto px-3 py-2 [scrollbar-width:none] sm:px-5">
          {JOURNEY.map(({ label, icon: Icon, href, tone }) => (
            <button key={label} onClick={() => navigate(href)} className={`flex shrink-0 items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-semibold text-white/50 hover:bg-white/5 hover:text-white ${diasporaTheme.focus}`}>
              <Icon className={`h-3.5 w-3.5 ${tone.split(" ").at(-1)}`} />
              {label}
            </button>
          ))}
        </div>
      ...
      <main className="mx-auto max-w-3xl space-y-8 px-4 pt-6 sm:px-6">
        <section>
          <div className="mb-3 flex items-end justify-between gap-3">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-teal-300/70">The living map</p>
              <h2 className="mt-1 text-xl font-black text-white">Diaspora Globe</h2>
            </div>
            <button onClick={() => navigate("/diaspora/heritage/globe")} className={`flex items-center gap-1 text-xs font-bold text-teal-300 ${diasporaTheme.focus}`}>
              Enter globe <ArrowRight className="h-3.5 w-3.5" />
            </button>
          </div>

          <button
            onClick={() => navigate("/diaspora/heritage/globe")}
            className={`group relative w-full overflow-hidden rounded-[2rem] border border-teal-200/20 bg-gradient-to-br from-[#082b29] via-[#063f3b] to-[#071a19] p-5 text-left shadow-[0_24px_80px_rgba(13,148,136,0.14)] sm:p-7 ${diasporaTheme.focus}`}
          >
            ...
            <div className="relative grid gap-7 sm:grid-cols-[1fr_auto] sm:items-end">
              <div className="max-w-xl">
                <span className="inline-flex items-center gap-2 rounded-full border border-teal-100/10 bg-teal-100/5 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-teal-100/70">
                  <Globe2 className="h-3 w-3" /> Connected world
                </span>
                <h3 className="mt-4 text-2xl font-black text-white sm:text-3xl">From homeland to home base.</h3>
                <p className="mt-2 max-w-lg text-sm leading-relaxed text-teal-50/60">
                  Explore living diaspora hubs, migration arcs, Griot stories, and communities connected through Niakofa.
                </p>
              ...
              <div className="flex h-24 w-24 items-center justify-center rounded-full border border-teal-100/10 bg-teal-100/[0.04] sm:h-32 sm:w-32">
                <Globe2 className="h-14 w-14 text-teal-200/75 sm:h-20 sm:w-20" />
              </div>
            </div>
          </button>
        </section>
        ...
        <section>
          <div className="mb-3">
            <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-amber-300/70">One journey</p>
            <h2 className="mt-1 text-xl font-black text-white">Preserve. Connect. Discover.</h2>
          </div>
          <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
            {JOURNEY.map(({ label, note, icon: Icon, href, tone }) => (
              <button key={label} onClick={() => navigate(href)} className={`rounded-2xl border ${tone.split(" ")[0]} ${tone.split(" ")[1]} p-4 text-left ${diasporaTheme.focus}`}>
                <Icon className={`h-5 w-5 ${tone.split(" ").at(-1)}`} />
        ...
        <section>
          <div className="mb-3 flex items-end justify-between gap-3">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-rose-300/70">Your family story</p>
              <h2 className="mt-1 text-xl font-black text-white">Recent activity</h2>
            </div>
            <button onClick={() => navigate("/diaspora/family")} className={`text-xs font-bold text-teal-300 ${diasporaTheme.focus}`}>Open vault</button>
          </div>
          {loading ? (
            <div className={`${diasporaTheme.radius} flex items-center justify-center border border-white/10 bg-white/[0.03] py-12`}><Loader2 className="h-5 w-5 animate-spin text-teal-300" /></div>
          ) : activity.length === 0 ? (
            <div className={`${diasporaTheme.radius} border border-dashed border-white/10 bg-white/[0.025] p-7 text-center`}>
              <Clock3 className="mx-auto h-8 w-8 text-white/20" />
              <p className="mt-3 text-sm font-bold text-white/70">Your story starts here.</p>
              <p className="mx-auto mt-1 max-w-sm text-xs leading-relaxed text-white/40">Preserve a memory, record an elder, or build your family tree.</p>
              <button onClick={() => navigate(oralHistoryHref)} className="mt-4 rounded-xl bg-amber-300 px-4 py-2 text-xs font-black text-[#201600]">Record a story</button>
            </div>
          ) : (
            <div className="space-y-2">
              {activity.slice(0, 5).map(item => (
                <button key={`${item.family_id}-${item.memory_id ?? item.time}-${item.title}`} onClick={() => navigate(item.memory_id ? `/family/${item.family_id}/memory/${item.memory_id}` : `/diaspora/vault/${item.family_id}`)} className={`flex w-full items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.035] p-3.5 text-left hover:bg-white/[0.06] ${diasporaTheme.focus}`}>
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-teal-300/10"><BookHeart className="h-4 w-4 text-teal-300" /></span>
                  <span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-white/85">{item.title}</span><span className="mt-0.5 block text-[11px] text-white/35">{relativeTime(item.time)}</span></span>
        ...
        <section className="grid gap-3 sm:grid-cols-2">
          <button onClick={() => navigate("/diaspora/dna")} className="rounded-2xl border border-blue-300/15 bg-blue-300/[0.05] p-5 text-left ${diasporaTheme.focus}">
            <Dna className="h-5 w-5 text-blue-300" />
            <p className="mt-3 text-sm font-bold text-white">DNA Connections</p>
            <p className="mt-1 text-xs leading-relaxed text-white/40">Connect a supported dataset. No match or ethnicity result is shown unless a real source provides it.</p>
            <span className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-blue-300">Connect data <ArrowRight className="h-3.5 w-3.5" /></span>
          </button>
          <button onClick={() => window.openNia?.("Help me preserve a family story.")} className="rounded-2xl border border-amber-300/15 bg-gradient-to-br from-amber-300/[0.08] to-teal-300/[0.04] p-5 text-left ${diasporaTheme.focus}">
            <Sparkles className="h-5 w-5 text-amber-300" />
            <p className="mt-3 text-sm font-bold text-white">Ask Nia</p>
            <p className="mt-1 text-xs leading-relaxed text-white/40">Use the global Nia guide without duplicating the full chat interface on the dashboard.</p>
            <span className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-amber-300">Open Nia <ArrowRight className="h-3.5 w-3.5" /></span>
          </button>
        </section>

        <footer className="border-t border-white/10 py-6 text-center">
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-amber-300/70">NIAKOFA</p>
          <p className="mt-1 text-xs italic text-white/30">Life · Community · Purpose</p>
        </footer>
      </main>
    </div>
  );
