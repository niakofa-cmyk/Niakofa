  ...
  return (
    <div className="min-h-screen bg-[#071312] pb-28 text-foreground">
      ...
      <main className="mx-auto max-w-2xl space-y-7 px-4 pt-6">
        ...
        <section><div className="mb-3"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-300/70">Preserve + connect</p><h2 className="mt-1 text-lg font-bold text-white">Start with what matters.</h2></div><div className="grid grid-cols-2 gap-2.5">{QUICK_ACTIONS.map((action) => { const Icon = action.icon; const href = action.label === "Oral History" ? buildOralHistoryHref(activity[0]?.family_id) : action.href; return <button key={action.label} onClick={() => navigate(href)} className={`rounded-2xl border p-4 text-left transition-transform active:scale-[0.98] ${action.className}`}><Icon className="h-5 w-5" /><p className="mt-3 text-sm font-bold">{action.label}</p><p className="mt-1 text-[11px] leading-relaxed text-white/45">{action.description}</p></button>; })}</div></section>
