...
export default function LegacyHomePage() {
  ...
  // ── Live Co-op Readiness (real data from /api/legacy/coop/:familyId/readiness) ──
  const [coopReady, setCoopReady] = useState<{
    minRequired: number;
    onlineCount: number;
    totalMembers: number;
    coopReady: boolean;
    onlineMembers: Array<{ memberId: number; displayName: string; role: string; avatarUrl: string | null }>;
  } | null>(null);
  ...
  const loadLegacyEngine = useCallback(async (familyId: number) => {
    ...
    try {
      ...
      const [
        ...
        fetch(`/api/legacy/coop/${familyId}/readiness`, { headers: authHeaders() }).catch(() => null),
      ]);
        ...
        <div className="max-w-lg mx-auto">
          ...
          {activeMode === "reunion" && (
            <div className="px-4 mb-5">
              ...
              {/* Live Co-op Readiness */}
              <div className={`rounded-2xl p-4 shadow-lg mb-3 border ${coopReady?.coopReady ? "bg-emerald-900/15 border-emerald-700/30" : "bg-[#2A1A0F] border-amber-800/30"}`}>
                <div className="flex items-center gap-3 mb-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${coopReady?.coopReady ? "bg-emerald-500/15" : "bg-rose-500/10"}`}>
                    {coopReady?.coopReady
                      ? <CheckCircle2 className="w-5 h-5 text-emerald-400" />
                      : <Users className="w-5 h-5 text-rose-400" />}
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-bold text-amber-100">Live Co-op</p>
                    <p className="text-xs text-amber-600 mt-0.5">
                      {coopReady
                        ? `${coopReady.onlineCount} of ${coopReady.totalMembers} family members online — ${coopReady.coopReady ? "Ready to play together!" : `Need ${coopReady.minRequired} online to start.`}`
                        : "Checking who's online..."}
                    </p>
                  </div>
                  {coopReady?.coopReady && (
                    <span className="text-[10px] font-black uppercase tracking-wide text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded-full">Ready</span>
                  )}
                </div>
                {coopReady && coopReady.onlineMembers.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-3">
                    {coopReady.onlineMembers.map((m) => (
                      <div key={m.memberId} className="flex items-center gap-1.5 bg-[#3A2A1A] rounded-full pl-2 pr-3 py-1">
                        <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                        <span className="text-xs text-amber-200">{m.displayName}</span>
                        {m.role && <span className="text-[10px] text-amber-600 capitalize">{m.role}</span>}
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="bg-[#2A1A0F] border border-amber-800/30 rounded-2xl p-4 shadow-lg mb-3">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-bold text-amber-100">World Regeneration</p>
                    <p className="text-xs text-amber-600 mt-0.5">Every contribution changes your family's world.</p>
                  </div>
                </div>
                <div className="space-y-2">
                  {[
                    { upload: "Oral Story", regenerates: "Dialogue, journal entries, side quests" },
                    { upload: "New Ancestor", regenerates: "Character roster, chapters, relationship graph" },
                    { upload: "Family Photo", regenerates: "Collectibles, locations, memories, NPC knowledge" },
                    { upload: "Landmark", regenerates: "Exploration map, travel quests, AR visits" },
                    { upload: "Recipe", regenerates: "Cooking quests, cultural traditions, achievements" },
                    { upload: "Letter", regenerates: "Dialogue, timeline, journal, collectibles" },
                    { upload: "New Cousin", regenerates: "Co-op activities, shared missions, reunion events" },
                  ].map((rule, i) => (
                    <div key={i} className="flex items-start gap-2 text-xs">
                      <span className="text-amber-400 font-bold flex-shrink-0 w-20">{rule.upload}</span>
                      <ChevronRight className="w-3 h-3 text-amber-700 flex-shrink-0 mt-0.5" />
                      <span className="text-amber-600 flex-1">{rule.regenerates}</span>
                    </div>
                  ))}
                </div>
              </div>
