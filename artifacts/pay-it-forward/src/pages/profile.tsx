  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        {/*
          overflow-y-auto moved to this inner div (was on the outer sheet) so
          the header stays pinned while the body scrolls, and the extra
          bottom padding (fixed space + real safe-area inset, combined in one
          value since separate padding-bottom classes would just override
          each other) guarantees the last row/button in every card — e.g. the
          last FAQ in "Help center" or the last guideline in "Community
          guidelines" — always has clear space above the home indicator
          instead of ending flush against the edge of the screen.
        */}
        <div className="p-5 pb-[calc(2.5rem+env(safe-area-inset-bottom,0px))] overflow-y-auto">{children}</div>
      </motion.div>
    </AnimatePresence>
  );
}
...
// ── Accessibility Card ─────────────────────────────────────────────────────
// Lets users override the OS "Reduce Motion" setting for bird animations.
// Many iOS users have "Reduce Motion" enabled for system UI (stops parallax
// and transition animations), but still want to see the Sankofa bird's
// Phase 3 flight effects. This toggle writes to localStorage and to the
// html[data-bird-anim="enabled"] attribute that gates the CSS suppression block.
function AccessibilityCard() {
  const { animEnabled, toggleAnim } = useAnimationPreference();
  // Detect OS Reduce Motion in real time so the description stays accurate
  // even if the user changes the system setting while the page is open.
  const osReducedMotion = useOsReducedMotion();

  // Build a contextual description so the toggle's purpose is always clear.
  let description: string;
  if (animEnabled) {
    description = "Full animations enabled — tap to respect your OS Reduce Motion setting";
  } else if (osReducedMotion) {
    description =
      "Your OS Reduce Motion is on — bird animations are suppressed. Tap to override and show full animations.";
  } else {
    description =
      "Animations are active (your OS Reduce Motion is off). This toggle only matters when OS Reduce Motion is enabled.";
  }
    ...
    <div className="bg-card border border-border rounded-2xl overflow-hidden">
      <div className="p-4 border-b border-border">
        <div className="font-black text-sm uppercase tracking-wider flex items-center gap-2">
          <Eye className="w-4 h-4 text-primary" /> Accessibility
        </div>
        <p className="text-xs text-muted-foreground mt-1">Motion and animation preferences</p>
      </div>
      <div className="divide-y divide-border">
        <div className="flex items-center justify-between gap-4 p-4">
          <div className="flex-1 min-w-0">
            <div className="text-sm font-semibold">Bird flight animations</div>
            <div className="text-xs text-muted-foreground mt-0.5">{description}</div>
          </div>
          <Switch
            checked={animEnabled}
            onCheckedChange={toggleAnim}
            aria-label="Enable bird flight animations regardless of OS Reduce Motion setting"
          />
        </div>
      </div>
    </div>
...
function RecentHelpersSection({
  ...
  // hasLoadedRef: spinner only shows on the very first load (no data yet).
  // Subsequent re-fetches (userId stable, component remounts) run silently
  // in the background without blanking the existing list — prevents flash-empty.
  ...
  useEffect(() => {
    ...
    fetch(`${base}/api/requests?requester_id=${userId}&status=completed&limit=6`)
      ...
      .catch(() => {
        ...
        // before — nothing disappears that was previously rendered.
      })
      .finally(() => setLoading(false));
  }, [userId]);
...
export default function ProfileScreen() {
  ...
  // Running repayment balance widget — same hook wallet.tsx uses to compute
  // per-request outstanding (pledge_amount - pledge_paid); aggregated here
  // into one overall "$X of $Y repaid" figure.
  const { data: outstandingPledgeRequests = [] } = useGetUserOutstandingPledges(userId ?? 0, {
    query: { enabled: !!userId, queryKey: getGetUserOutstandingPledgesQueryKey(userId ?? 0), staleTime: 30000 }
  });
  const outstandingPledgeTotals = (() => {
    const pledgedTotal = outstandingPledgeRequests.reduce((sum, r) => sum + (r.pledge_amount ?? 0), 0);
    const paidTotal = outstandingPledgeRequests.reduce((sum, r) => sum + (r.pledge_paid ?? 0), 0);
    const pct = pledgedTotal > 0 ? Math.min(100, Math.round((paidTotal / pledgedTotal) * 100)) : 0;
      ...
      <div className="flex-1 max-w-lg mx-auto w-full p-4 space-y-4">
        ...
        {tab === "overview" && (
          <>
              ...
              <div className="relative">
                ...
                {helperModeActive && (
                  <div className="absolute -top-1 -right-1 w-5 h-5 bg-green-500 rounded-full border-2 border-background flex items-center justify-center pointer-events-none">
                    <div className={`w-2 h-2 bg-white rounded-full${suppressed ? "" : " animate-pulse"}`} />
                  </div>
                )}
                <input
                  ref={avatarInputRef}
            ...
            {/* Running Repayment Balance — same outstanding-pledges hook wallet.tsx
                uses, aggregated into one "$X of $Y repaid" line so a user can see
                their overall standing without opening the wallet page. */}
            {outstandingPledgeTotals.pledgedTotal > 0 && (
              <button
                onClick={() => setLocation("/wallet")}
                className="w-full bg-card border border-border rounded-2xl p-4 space-y-2 text-left"
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-black uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                    <DollarSign className="w-3.5 h-3.5 text-primary" /> Pledge Repayment
                  </span>
                  <span className="text-[10px] font-black text-muted-foreground">
                    {outstandingPledgeTotals.pct}%
                  </span>
                </div>
                <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
                  <div
                    className="h-full bg-primary rounded-full transition-all"
                    style={{ width: `${outstandingPledgeTotals.pct}%` }}
                  />
                </div>
                <div className="text-sm">
                  <span className="font-black text-primary">${outstandingPledgeTotals.paidTotal.toFixed(2)}</span>
                  <span className="text-muted-foreground"> of ${outstandingPledgeTotals.pledgedTotal.toFixed(2)} repaid</span>
                </div>
              </button>
            )}
            ...
            {/* Helper Mode */}
            {/* Bug fixed: this toggle used to render unconditionally for every
                user, regardless of helper_status. The server has always
                required helper_status === "approved" to accept "active: true"
                (PATCH /users/:id/helper-mode) — going through the helper
                application + admin review first is by design, not optional.
                Anyone who hadn't been through that (the vast majority of new
                users, since helper_status defaults to "offline") saw a
                toggle that would always be rejected, with no indication why.
                Now the card itself reflects the real state instead of
                offering a control that's guaranteed to fail. */}
            {currentUser.helper_status === "approved" ? (
              <div className="bg-card border border-border rounded-2xl p-4 flex items-center justify-between">
                <div>
                  <div className="font-bold flex items-center gap-2">
                    Helper Mode
                    {helperModeActive && <span className={`w-2 h-2 rounded-full bg-green-500${suppressed ? "" : " animate-pulse"}`} />}
                  </div>
            ...
            ) : currentUser.helper_status === "pending" ? (
              <div className="bg-card border border-border rounded-2xl p-4">
                <div className="font-bold flex items-center gap-2">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  We'll notify you as soon as it's approved — then you can go online and start
                  receiving nearby help requests.
                </p>
              </div>
            ) : (
              <div className="bg-card border border-border rounded-2xl p-4">
                <div className="font-bold">Become a Helper</div>
                <p className="text-sm text-muted-foreground mt-1 mb-3">
                  Apply to start receiving nearby help requests and get paid — a quick approval
                  step comes first so requesters and other helpers can trust who they're working with.
                </p>
                <Button onClick={() => setLocation("/helper-onboarding")} className="w-full">
        ...
        {tab === "settings" && (
          <div className="space-y-3">
            ...
            {/* Accessibility — bird animation override */}
            <AccessibilityCard />
            ...
            <div className="bg-card border border-border rounded-2xl overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                {[
                  ...
                  { label: "Government ID", done: (currentUser as import("@/lib/userTypes").ExtendedUser).identity_verified, desc: (currentUser as import("@/lib/userTypes").ExtendedUser).identity_verified ? "Verified" : "Required for emergency helper status" },
                  { label: "Background check", done: (currentUser as import("@/lib/userTypes").ExtendedUser).background_check_status === "completed", desc: (currentUser as import("@/lib/userTypes").ExtendedUser).background_check_status === "completed" ? "Completed" : "Required for Trusted Helper badge" },
                ].map(item => (
