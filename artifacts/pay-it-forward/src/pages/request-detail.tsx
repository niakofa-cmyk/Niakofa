    ...
    <div className="min-h-[100dvh] bg-background">
      ...
      <div className="px-4 pt-5 pb-safe pb-8 space-y-4 max-w-lg mx-auto">
        ...
        <div className="space-y-2 pt-2">
          ...
          {/* Requester: always show track button once a helper is assigned */}
          {isRequester && request.helper_id && (
            <Button
              className="w-full h-12 font-black gap-2 bg-green-600 hover:bg-green-700 text-white"
              onClick={() => setLocation(`/request/${requestId}/track`)}
            >
              <Navigation2 className="w-4 h-4" />
              Track Your Helper Live
            </Button>
          )}
          {isRequester && !request.helper_id && isOpen && (
            <Button variant="outline" className="w-full h-12 font-black" onClick={() => setLocation(`/request/${requestId}/track`)}>
              Track Your Request
            </Button>
          )}
          {isHelper && (request.status === "en_route" || request.status === "claimed" || request.status === "arrived") && (
            <Button className="w-full h-12 font-black gap-2" onClick={() => setLocation(`/request/${requestId}`)}>
              <Navigation2 className="w-4 h-4" />
              Continue Helping
            </Button>
          )}
          ...
          {/* Tip button — shown to the requester once a helper is assigned and
              the request is completed or arrived. Uses wallet balance. */}
          {isRequester && request.helper_id && (
            (request.status === "completed" || request.status === "arrived") && (
              <button
                onClick={() => setShowTip(true)}
                className="w-full h-12 flex items-center justify-center gap-2 bg-yellow-500/10 border border-yellow-500/30 rounded-2xl font-black text-sm text-yellow-400 active:scale-[0.97] transition-all"
              >
                <DollarSign className="w-4 h-4" />
                Send a Tip to {request.helper_name ?? "Helper"}
              </button>
            )
          )}
        </div>
