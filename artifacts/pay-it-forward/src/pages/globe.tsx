...
export default function GlobePage() {
  ...
  // Delete-story state — author-only; shows confirm prompt before calling API
  const [deleteConfirmStoryId, setDeleteConfirmStoryId] = useState<number | null>(null);
...
// eslint-disable-next-line @typescript-eslint/no-unused-expressions
  ...
  // DELETE /api/griot/stories/:id — author or admin only
  const deleteStory = async (storyId: number) => {
    setIsDeletingStory(true);
    try {
      const res = await fetch(`/api/griot/stories/${storyId}`, {
        method: "DELETE",
        headers: authHeaders(),
      });
      if (res.ok) {
        ...
        // If we deleted the story currently being reviewed, close panel or switch
        if (reviewingStoryId === storyId) {
          const remaining = myStories.filter(s => s.id !== storyId);
          if (remaining.length > 0) {
            setReviewingStoryId(remaining[0].id);
          } else {
            setPanel(null);
          }
        }
        setDeleteConfirmStoryId(null);
        // Refresh the global stories feed so the pin disappears from the map
        fetchStories();
      } else {
        const errData = await res.json().catch(() => ({})) as { error?: string };
        toast({ title: errData.error ?? "Failed to delete story", variant: "destructive" });
      }
    } catch {
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-1">
                  {/* Delete-story button — visible only to the author */}
                  {currentUser && myStories.find(s => s.id === reviewingStoryId)?.author_id === currentUser.id && (
                    <button
                      onClick={() => setDeleteConfirmStoryId(reviewingStoryId)}
              ...
              {/* Story selector — shown when author has multiple stories */}
              {myStories.length > 1 && (
                <div className="flex gap-2 mb-3 overflow-x-auto pb-1">
                  {myStories.map(s => (
                    <button
                      key={s.id}
                      onClick={() => { setReviewingStoryId(s.id); fetchMyTranslations(s.id); }}
                      className={`shrink-0 px-3 py-1.5 rounded-full text-[11px] font-bold border transition-all ${
                        s.id === reviewingStoryId
              ...
              {deleteConfirmStoryId === reviewingStoryId && (
                <div className="mb-4 p-3 bg-destructive/10 border border-destructive/30 rounded-xl flex items-start gap-3">
                  <AlertTriangle className="w-4 h-4 text-destructive shrink-0 mt-0.5" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-black text-destructive">Delete this story?</p>
                    <p className="text-[11px] text-muted-foreground mt-0.5">
                      This will permanently remove the story and all its translations from the map.
                    </p>
                    <div className="flex gap-2 mt-3">
                      <button
                        onClick={() => deleteStory(reviewingStoryId)}
                        disabled={isDeletingStory}
                        className="px-4 py-1.5 bg-destructive text-destructive-foreground rounded-lg text-xs font-black disabled:opacity-50 active:scale-[0.97] transition-all"
                      >
                        {isDeletingStory ? "Deleting…" : "Yes, delete"}
                      </button>
                      <button
                        onClick={() => setDeleteConfirmStoryId(null)}
                        disabled={isDeletingStory}
                        className="px-4 py-1.5 bg-muted text-foreground rounded-lg text-xs font-bold active:scale-[0.97] transition-all"
                      >
                        Cancel
                      </button>
                    </div>
