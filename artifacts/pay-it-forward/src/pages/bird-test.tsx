...
import { PhaseAuditSuite } from "./bird-phase-audit";
import {
  SankofaBird360Pipeline,
  SankofaBirdTurnSequence,
  SankofaBirdTurnSequenceGrid,
  SankofaBirdWingDeformRow,
  SankofaBirdTailDeformRow,
  SankofaBirdCrossScreenDemo,
} from "@/components/SankofaBirdSprite360";
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        {/* ── 360° Asset Pipeline — Full multi-angle view system ───────── */}
        <SectionLabel>360° Asset Pipeline — Full multi-angle SVG view system (15 views + deformation)</SectionLabel>
        <p className="text-xs mb-4" style={{ color: "rgba(255,255,255,0.4)", maxWidth: 700 }}>
          <strong style={{ color: "rgba(0,212,255,0.8)" }}>360° Pipeline</strong>: Complete directional asset
          system matching the Official SVG Asset Pipeline reference. 8 cardinal + 3/4 views, 7 vertical/diagonal/cross
          views, 12-step illusion-perspective turn sequence, 5 wing deformation states, 4 tail deformation states.
          All derived from the same SVG layer vocabulary via CSS transform skew/scale — 2.5D illusion of 3D rotation
          through foreshortening + layered path-morphing. No WebGL. Visual reference: public/SANKOFA_BIRD_OFFICIAL_REFERENCE.png
        </p>
        <div className="mb-6">
          <div className="flex flex-wrap gap-8 mb-6">
            <div>
              <p className="text-xs mb-2" style={{ color: "rgba(0,212,255,0.6)" }}>Live Turn Sequence (auto-play)</p>
              <SankofaBirdTurnSequence size={80} stepMs={700} />
            </div>
            <div>
              <p className="text-xs mb-2" style={{ color: "rgba(0,212,255,0.6)" }}>Cross-Screen Flight (heading-synced)</p>
              <SankofaBirdCrossScreenDemo width={400} height={100} size={60} />
            </div>
          </div>
          <SankofaBirdTurnSequenceGrid tileSize={68} />
          <div className="mt-6 space-y-6">
            <SankofaBirdWingDeformRow tileSize={80} />
            <SankofaBirdTailDeformRow tileSize={80} />
          </div>
          <div className="mt-8">
            <p className="text-xs mb-3" style={{ color: "rgba(0,212,255,0.6)" }}>Full Asset Pipeline Spritesheet</p>
            <SankofaBird360Pipeline tileSize={80} />
          </div>
        </div>

        {/* ── 360° Directional Turn Demo ────────────────────────────────── */}
        <SectionLabel>360° Directional Turning — head, neck &amp; body face direction of flight</SectionLabel>
        <p className="text-xs mb-4" style={{ color: "rgba(255,255,255,0.4)", maxWidth: 700 }}>
          <strong style={{ color: "rgba(0,212,255,0.8)" }}>P17.0 + P17.1 + P17.2</strong>: Bird faces the
          direction of travel at all 16 compass headings. When heading is in the rightward half (10°–170°)
          the SVG is horizontally mirrored (<code style={{ color: "rgba(255,255,255,0.6)" }}>data-facing="right"</code>).
          Head/neck CSS var signs are negated in JS so bank-driven rotations stay visually correct after the flip.
          Transform-origin fixed to the anatomical neck-body junction (18px, 16px in SVG view-box) — the previous
          "center bottom" was (20px, 40px) producing invisible rotations. Vertical gaze amplified to ±14°/±12°
          (was ±8°/±7°) so up/down head tilt reads clearly at 48px map-marker scale.
        </p>
        <DirectionalTurnDemo />

        {/* ── Phase 20: Multi-View 360° Sprite Cross-Fade ──────────────── */}
        <SectionLabel>Phase 20 — 360° Multi-View Sprite System (Front · Side · Back cross-fade)</SectionLabel>
        <p className="text-xs mb-4" style={{ color: "rgba(255,255,255,0.4)", maxWidth: 720 }}>
          <strong style={{ color: "rgba(0,212,255,0.8)" }}>Phase 20</strong>: Three bird sprites
          (FrontView · SideView · BackView) sit absolutely inside the rig and cross-fade based on
          compass heading. Zones: Front (N ±22.5°), 45° linear cross-fades at NE/SE/SW/NW,
          Side (E ±22.5°, W ±22.5°), Back (S ±22.5°). Walking legs (0.55 s alternating stride)
          animate at all three views when <code style={{ color: "rgba(255,255,255,0.6)" }}>data-speed="walking"</code>.
        </p>
        <MultiView360Demo />
        ...
        <SectionLabel>Phase 12 — Real-time 8-direction gaze system</SectionLabel>
        <p className="text-xs mb-4" style={{ color: "rgba(255,255,255,0.4)" }}>
          computeGazeVector() drives data-gaze on the rig. Iris, head, neck, and body all
          follow the gaze direction organically. Auto-saccade micro-drifts at idle.
        </p>
        <GazeDirectionDemo />

        <SectionLabel>Phase 12 — Gap closure (10 remaining audit items)</SectionLabel>
        <p className="text-xs mb-4" style={{ color: "rgba(255,255,255,0.4)" }}>
          Shadow coloring · body lean anticipation · thermal lift · iris alert saccade ·
          per-feather flight oscillation · leading-edge compression · helping low-zoom ·
          speed-correlated crown · wing pre-extension.
        </p>
        <GapClosureDemo />

        {/* ── Phase 13: Full Authentic Aerodynamics ─────────────────────── */}
        <SectionLabel>Phase 13 — Full Authentic Aerodynamics</SectionLabel>
        <p className="text-xs mb-4" style={{ color: "rgba(255,255,255,0.4)", maxWidth: 700 }}>
          <strong style={{ color: "rgba(80,200,120,0.9)" }}>P13.1–P13.12</strong>: twelve real avian biomechanics
          effects. (P13.1) Figure-8 oval wing stroke: large downstroke + small upstroke with lateral sway
          at running/driving speeds. (P13.2) WAIR (Wing-Assisted Incline Running): body pitched 28°, wings
          churning, legs driving, head horizon-locked. (P13.3) Dynamic soaring: 4.2s albatross
          dive-climb energy-exchange cycle. (P13.4) Mating display: 1.6s courtship pivot + wing fan +
          head bow + tail spread + crown flash. (P13.5) Hover wrist rotation: figure-8 wrist stroke so
          the bird generates lift on both up- and downstroke. (P13.6) Individual knee articulation:
          tibiotarsus-tarsometatarsus bend with alternating L/R cadence at walking/running speed and
          bilateral impact flex on landing. (P13.7) Murmuration iridescence wave: hue-rotate shimmer
          travelling across both wings at driving speed. (P13.8) Full body gaze chain: lateral gaze now
          also rolls the shoulders and deflects the tail as a rudder. (P13.9) Leading-edge feather slot:
          primary feather tips separate on takeoff/hover to model the aerodynamic slot.
          (P13.10–P13.12) Knee flex pulse, wrist stroke pulse, and per-feather micro-oscillation sharpening.
        </p>

        <p className="text-[10px] mb-3" style={{ color: "rgba(255,255,255,0.25)" }}>
          All P13 effects respect prefers-reduced-motion and battery-saver LOD3.
          WAIR, soaring, and mating-display are mutually exclusive (each overrides the others).
        </p>

        <SectionLabel>P13.1 — Figure-8 wing stroke</SectionLabel>
        <FigureEightDemo />

        <SectionLabel>P13.2 — WAIR (Wing-Assisted Incline Running)</SectionLabel>
        <WairDemo />

        <SectionLabel>P13.3 — Dynamic soaring</SectionLabel>
        <SoaringDemo />

        <SectionLabel>P13.4 — Mating display</SectionLabel>
        <MatingDisplayDemo />

        <SectionLabel>P13.5+P13.6 — Hover wrist rotation + knee articulation</SectionLabel>
        <HoverAndKneeDemo />

        <SectionLabel>P13.8 — Full body gaze chain (shoulder + tail)</SectionLabel>
        <GazeBodyChainDemo />

        <SectionLabel>Phase 13 — All aerodynamic states side-by-side</SectionLabel>
        <AerodynamicsPhase13Panel />
        <p className="text-[10px] mt-2 mb-8" style={{ color: "rgba(255,255,255,0.22)" }}>
          P13.7 murmur wave fires automatically at data-speed=driving (14 m/s) — visible in the
          Murmur Wave card and SpeedRampDemo above. P13.9 feather slots are visible at takeoff/hover.
          P13.11 knee joint dots appear at zoom&nbsp;12+ (mid/high/street LOD).
        </p>

        {/* ── Phase 14: Real-time 8-direction gaze control ─────────────── */}
        <SectionLabel>Phase 14 — Real-time 8-direction gaze (all compass points active)</SectionLabel>
        <p className="text-xs mb-3" style={{ color: "rgba(255,255,255,0.35)" }}>
          Saccade expanded from 4 to 8 phases — all compass gaze directions now fire during idle drift.
          Banking-responsive gaze: hard turns (&gt;10°) shift iris left/right even without nav route data.
          Live cycling demo auto-advances every 1.5 s through all 9 states. All gaze rules are CSS — no React re-renders.
        </p>
        <GazeRealTimeDemo />

        {/* ── Activity-level controls ────────────────────────────────────── */}
        <SectionLabel>Activity-level controls — crown sway speed + blink rate</SectionLabel>
        <p className="text-xs mb-3" style={{ color: "rgba(255,255,255,0.35)" }}>
          Drag the slider to any activity level and see crown sway, blink rate, and glow intensity respond in real time.
          Click any tier card to snap to that preset. Also lets you verify night-mode plumage at any activity level.
        </p>
        <ActivityLevelControlDemo />

        {/* ── Solar sky-tier simulation ──────────────────────────────────── */}
        <SectionLabel>Solar sky-tier simulation — sunset transition (no waiting required)</SectionLabel>
        <p className="text-xs mb-3" style={{ color: "rgba(255,255,255,0.35)" }}>
          Sun elevation slider simulates useSolarTier() thresholds (day &gt;10° · golden 0–10° · twilight −6–0° · night &lt;−6°).
          Verify night-mode plumage without waiting for real sunset. Click any tier card to jump directly.
        </p>
        <SolarSimDemo />

        {/* ── Phase Audit Suite ─────────────────────────────────────────── */}
        <SectionLabel>Phase Audit Suite — per-phase animation verification (Phase 1–13)</SectionLabel>
        <p className="text-xs mb-3" style={{ color: "rgba(255,255,255,0.35)" }}>
          Every phase tab renders all of that phase's animations, data-* attributes, CSS vars, and rig structures
          simultaneously — live, no mocks. Use ← → arrow keys or the tab bar to navigate. "Run All" cycles through
          all 13 phases at 6 s each so you can verify the full suite hands-free.
        </p>
        <PhaseAuditSuite />

        {/* ── Phase 20 SME Physics harness ─────────────────────────────── */}
        <SectionLabel>Phase 20 — SME Physics harness (dynamic neck bezier + eye tracking)</SectionLabel>
        <p className="text-xs mb-3" style={{ color: "rgba(255,255,255,0.35)" }}>
          Live gauges for all 8 Sankofa Motion Engine physics channels written each rAF frame.
          Dynamic neck bezier bends toward head direction. Eyes track travel direction on screen.
          No Rive .riv file required — all animation runs from pure JS/CSS.
        </p>
        <Phase20SmePhysicsDemo />

        {/* ── Regression checklist ──────────────────────────────────────── */}
        <SectionLabel>Regression checklist — all critical states simultaneously</SectionLabel>
        <p className="text-xs mb-3" style={{ color: "rgba(255,255,255,0.35)" }}>
          Every critical bird state rendered at once. Scan visually — any frozen, absent, or broken animation is a regression.
          Banking cards use live heading delta so bank animation actually fires. No interaction needed.
        </p>
        <RegressionChecklistPanel />
...
/* ── 360° Directional Turn Demo ─────────────────────────────────────── */
/**
 * Shows the bird at 16 compass headings (0°–337.5° in 22.5° steps).
 * Verifies:
 *  • data-facing="right" flips the SVG via scaleX(-1) for headings 10°–170°
 *  • Head/neck rotations stay visually correct after the flip (signs negated in JS)
 *  • transform-origin 18px 16px (neck-body junction) gives visible head/neck curves
 *  • Vertical gaze amplified to ±14°/±12° reads clearly at 48px marker size
 */
function DirectionalTurnDemo() {
  const HEADINGS = [
    { deg: 0,     label: "N  0°",    navigating: true,  speed: 8 },
    { deg: 22.5,  label: "NNE 22°",  navigating: true,  speed: 8 },
    { deg: 45,    label: "NE  45°",  navigating: true,  speed: 8 },
    { deg: 67.5,  label: "ENE 67°",  navigating: true,  speed: 8 },
    { deg: 90,    label: "E   90°",  navigating: true,  speed: 8 },
    { deg: 112.5, label: "ESE 112°", navigating: true,  speed: 8 },
    { deg: 135,   label: "SE 135°",  navigating: true,  speed: 8 },
    { deg: 157.5, label: "SSE 157°", navigating: true,  speed: 8 },
    { deg: 180,   label: "S  180°",  navigating: true,  speed: 8 },
    { deg: 202.5, label: "SSW 202°", navigating: true,  speed: 8 },
    { deg: 225,   label: "SW 225°",  navigating: true,  speed: 8 },
    { deg: 247.5, label: "WSW 247°", navigating: true,  speed: 8 },
    { deg: 270,   label: "W  270°",  navigating: true,  speed: 8 },
    { deg: 292.5, label: "WNW 292°", navigating: true,  speed: 8 },
    { deg: 315,   label: "NW 315°",  navigating: true,  speed: 8 },
  ...
  const _GAZE_STATES = [
    ...
    { label: "Left",    heading: 270, speed: 0, navigating: false },
    { label: "Right",   heading: 90,  speed: 0, navigating: false },
  ];
  return (
    <div className="space-y-6">
      {/* Row 1: all 16 compass headings */}
      <div>
        <p className="text-xs mb-3" style={{ color: "rgba(255,255,255,0.5)" }}>
          All 16 compass headings at 8 m/s (running tier). Bird should face the
          direction of travel — head/beak toward the heading. Right-half headings
          (22°–157°) show <code style={{ color: "rgba(0,212,255,0.6)" }}>data-facing="right"</code>.
        </p>
        <div className="flex flex-wrap gap-3">
          {HEADINGS.map(({ deg, label, navigating, speed }) => {
            const facing = deg > 10 && deg < 170 ? "right" : "left";
            return (
              <div
                key={deg}
                className="flex flex-col items-center gap-2 rounded-xl p-3"
                style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", minWidth: 80 }}
              >
                <SankofaBird
                  heading={deg}
                  mapBearing={0}
                ...
                <span
                  ...
                  style={{
                    color: facing === "right" ? "rgba(0,212,255,0.9)" : "rgba(255,255,255,0.4)",
                    background: facing === "right" ? "rgba(0,212,255,0.12)" : "transparent",
                  }}
                >
                  {facing}
                </span>
      ...
      {/* Row 2: Gaze directions — head turn + vertical tilt */}
      <div>
        <p className="text-xs mb-3" style={{ color: "rgba(255,255,255,0.5)" }}>
          Gaze and approach states — head tilt visibility at idle. Vertical gaze
          amplified to ±14° (up) / ±12° (down) from the previous ±8°/±7°.
        </p>
        <div className="flex flex-wrap gap-3">
          <BirdCard label="Idle — forward" subLabel="no gaze, facing left" state={{ heading: 270, mapBearing: 0, speed: 0, navigating: false }} size={48} mapZoom={17} />
          <BirdCard label="Approach ↓" subLabel="looking down (±12°)" state={{ heading: 270, mapBearing: 0, speed: 4, navigating: true }} approaching size={48} mapZoom={17} />
          <BirdCard label="Gliding ↑" subLabel="looking up (±14°)" state={{ heading: 270, mapBearing: 0, speed: 55, navigating: true }} size={48} mapZoom={17} />
          <BirdCard label="Turn left gaze" subLabel="saccade-left at idle" state={{ heading: 270, mapBearing: 0, speed: 0, navigating: false }} size={48} mapZoom={17} upcomingTurnDirection="left" />
          <BirdCard label="Turn right gaze" subLabel="saccade-right at idle" state={{ heading: 270, mapBearing: 0, speed: 0, navigating: false }} size={48} mapZoom={17} upcomingTurnDirection="right" />
        </div>
      </div>

      {/* Row 3: Facing right with banking — verify sign correction */}
      <div>
        <p className="text-xs mb-3" style={{ color: "rgba(255,255,255,0.5)" }}>
          Right-facing bird (heading≈90°) with live banking — verifies that negated
          signs keep bank tilt visually correct after the scaleX(-1) flip.
        </p>
        <div className="flex flex-wrap gap-3">
          <BirdCard label="East — no bank" subLabel="90° heading, straight" state={{ heading: 90, mapBearing: 0, speed: 8, navigating: true }} size={48} mapZoom={14} />
          <BirdCard label="East → bank R" subLabel="turning right" state={{ heading: 100, mapBearing: 0, speed: 8, navigating: true }} size={48} mapZoom={14} upcomingTurnDirection="right" />
          <BirdCard label="East → bank L" subLabel="turning left" state={{ heading: 80, mapBearing: 0, speed: 8, navigating: true }} size={48} mapZoom={14} upcomingTurnDirection="left" />
          <BirdCard label="West — no bank" subLabel="270° heading, straight" state={{ heading: 270, mapBearing: 0, speed: 8, navigating: true }} size={48} mapZoom={14} />
          <BirdCard label="West → bank R" subLabel="turning right" state={{ heading: 280, mapBearing: 0, speed: 8, navigating: true }} size={48} mapZoom={14} upcomingTurnDirection="right" />
          <BirdCard label="West → bank L" subLabel="turning left" state={{ heading: 260, mapBearing: 0, speed: 8, navigating: true }} size={48} mapZoom={14} upcomingTurnDirection="left" />
        </div>
      </div>
    </div>
  );
...
/* ── Phase 12: Real-time 8-direction gaze demo ──────────────────────────── */
function GazeDirectionDemo() {
  const GAZE_STATES: Array<{
    label: string;
    subLabel: string;
    gaze?: "left" | "right" | "up" | "down" | "upleft" | "upright" | "downleft" | "downright";
    extra?: Partial<{ upcomingTurnDirection: "left" | "right"; approaching: boolean; newNotification: boolean }>;
  }> = [
    { label: "Gaze — forward",    subLabel: "neutral / default",                gaze: undefined },
    { label: "Gaze — left",       subLabel: "iris+head tilt left",              gaze: "left" },
    { label: "Gaze — right",      subLabel: "iris+head tilt right",             gaze: "right" },
    { label: "Gaze — up",         subLabel: "scanning horizon (glide)",         gaze: "up" },
    { label: "Gaze — down",       subLabel: "approaching destination",          gaze: "down",      extra: { approaching: true } },
    { label: "Gaze — upleft",     subLabel: "upcoming turn left anticipation",  gaze: "upleft",    extra: { upcomingTurnDirection: "left" } },
    { label: "Gaze — upright",    subLabel: "upcoming turn right anticipation", gaze: "upright",   extra: { upcomingTurnDirection: "right" } },
    { label: "Gaze — notification",subLabel: "alert saccade burst",             gaze: undefined,   extra: { newNotification: true } },
  ];

  return (
    <div className="flex flex-wrap gap-4 mb-8">
      {GAZE_STATES.map(({ label, subLabel, extra }) => (
        <BirdCard
          key={label}
          label={label}
          subLabel={subLabel}
          ...
          state={{
            ...
            mapBearing: 0,
            speed: 0,
            navigating: !!extra?.upcomingTurnDirection,
            newNotification: extra?.newNotification,
          }}
          mapZoom={17}
          size={52}
          upcomingTurnDirection={extra?.upcomingTurnDirection ?? null}
          approaching={extra?.approaching ?? false}
        />
      ))}
    </div>
  );
...
/* ── Phase 12: Gap-closure demo (G1-G10) ─────────────────────────────────── */
function GapClosureDemo() {
  const [sweepHeading, setSweepHeading] = useState(45);
  useEffect(() => {
    let h = 45, dir = 1, ticks = 0;
    const id = setInterval(() => {
      h = (h + dir * 4 + 360) % 360;
      ticks++;
      if (ticks >= 12) { dir = -dir; ticks = 0; }
      setSweepHeading(Math.round(h));
    }, 80);
    return () => clearInterval(id);
  }, []);
  ...
  return (
    <div className="flex flex-wrap gap-4 mb-8">
      {/* G2: Shadow gold/teal coloring */}
      <BirdCard label="G2 Shadow · helping" subLabel="gold drop-shadow pulse" badge="G2"
        state={{ heading: 0, mapBearing: 0, speed: 0, navigating: false }}
        mapZoom={17} size={52} isHelping={true} />
      <BirdCard label="G2 Shadow · celebrate" subLabel="teal shadow pulse" badge="G2"
        state={{ heading: 0, mapBearing: 0, speed: 0, navigating: false, celebrating: true }}
        mapZoom={17} size={52} />

      {/* G3+G10: Upcoming-turn body lean + wing pre-extension */}
      <BirdCard label="G3+G10 Pre-turn left" subLabel="body lean + wing preextend" badge="G3"
        state={{ heading: sweepHeading, mapBearing: 0, speed: 8, navigating: true }}
        mapZoom={17} size={52} upcomingTurnDirection="left" />
      <BirdCard label="G3+G10 Pre-turn right" subLabel="body lean + wing preextend" badge="G3"
        state={{ heading: sweepHeading, mapBearing: 0, speed: 8, navigating: true }}
        mapZoom={17} size={52} upcomingTurnDirection="right" />

      {/* G4: Thermal lift during glide */}
      <BirdCard label="G4 Thermal lift" subLabel="body rises 2.5px on updraft" badge="G4"
        state={{ heading: 0, mapBearing: 0, speed: 55, navigating: true }}
        mapZoom={17} size={52} />

      {/* G5: Eye iris alert saccade on notification */}
      <BirdCard label="G5 Iris saccade" subLabel="rapid 7-point eye scan on notification" badge="G5"
        state={{ heading: 0, mapBearing: 0, speed: 0, navigating: false, newNotification: true }}
        mapZoom={17} size={52} />

      {/* G6: Per-feather flight oscillation (visible at street zoom) */}
      <BirdCard label="G6 Feather microwave" subLabel="per-feather flight oscillation (street)" badge="G6"
        state={{ heading: 0, mapBearing: 0, speed: 8, navigating: true }}
        mapZoom={18} size={52} />

      {/* G7: Pre-bank feather compression */}
      <BirdCard label="G7 Leading-edge compress" subLabel="inside-turn primaries flatten" badge="G7"
        state={{ heading: 0, mapBearing: 0, speed: 8, navigating: true }}
        mapZoom={18} size={52} upcomingTurnDirection="left" />

      {/* G8: Helping at low zoom */}
      <BirdCard label="G8 Helping · low zoom" subLabel="gold tint visible at city scale" badge="G8"
        state={{ heading: 0, mapBearing: 0, speed: 5, navigating: true }}
        mapZoom={8} size={52} isHelping={true} />

      {/* G9: Speed-correlated crown sway */}
      <BirdCard label="G9 Crown · walking" subLabel="slow sway 4.5s" badge="G9"
        state={{ heading: 0, mapBearing: 0, speed: 1.4, navigating: true }}
        mapZoom={17} size={52} />
      <BirdCard label="G9 Crown · driving" subLabel="fast sway 1.4s" badge="G9"
        state={{ heading: 0, mapBearing: 0, speed: 14, navigating: true }}
        mapZoom={17} size={52} />
      <BirdCard label="G9 Crown · airplane" subLabel="rapid sway 0.7s" badge="G9"
        state={{ heading: 0, mapBearing: 0, speed: 55, navigating: true }}
        mapZoom={17} size={52} />
    </div>
  );
...
/* ── Phase 13: BirdCard wrapper with P13 props ────────────────────────────── */
function BirdCardP13({
  label,
  subLabel,
  badge,
  state,
  ...
  return (
    ...
    >
      ...
      <div className="text-center">
        <p className="text-xs font-semibold text-white leading-tight">{label}</p>
        <p className="text-[10px] mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>{subLabel}</p>
        {badge && (
          <span
            className="inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] font-semibold"
            style={{ background: "rgba(80,200,120,0.15)", color: "rgba(80,200,120,0.9)", border: "1px solid rgba(80,200,120,0.25)" }}
          >
            {badge}
          </span>
        )}
...
/* ── Phase 13: Figure-8 wing stroke demo ─────────────────────────────────── */
function FigureEightDemo() {
  const [speed, setSpeed] = useState(5);
  useEffect(() => {
    const id = setInterval(() => {
      setSpeed(s => {
        if (s >= 14) return 1.4;
        return +(s + 0.6).toFixed(1);
      });
    }, 200);
    return () => clearInterval(id);
  }, []);
  ...
  return (
    <div className="flex flex-wrap gap-4 mb-4">
      <BirdCardP13
        label="Figure-8 · walking"
        subLabel="1.4 m/s — standard flap arc"
        badge="P13.1"
        state={{ heading: 0, mapBearing: 0, speed: 1.4, navigating: true }}
      />
      <BirdCardP13
        label="Figure-8 · running"
        subLabel="5 m/s — oval stroke fires"
        badge="P13.1"
        state={{ heading: 0, mapBearing: 0, speed: 5, navigating: true }}
      />
      <BirdCardP13
        label="Figure-8 · driving"
        subLabel="14 m/s — tighter fast oval"
        badge="P13.1"
        state={{ heading: 0, mapBearing: 0, speed: 14, navigating: true }}
      />
      <div className="flex flex-col items-center gap-2 rounded-xl p-3"
        style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(0,212,255,0.12)", minWidth: 130 }}>
        <div style={{ width: 72, height: 72, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <SankofaBird heading={0} mapBearing={0} speed={speed} navigating={true} size={52} mapZoom={17} />
        </div>
        <div className="text-center">
          <p className="text-xs font-semibold text-white">Live speed ramp</p>
          <p className="text-[10px] mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>{speed.toFixed(1)} m/s</p>
          <span className="inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] font-semibold"
            style={{ background: "rgba(80,200,120,0.15)", color: "rgba(80,200,120,0.9)", border: "1px solid rgba(80,200,120,0.25)" }}>
            P13.1
          </span>
...
function WairDemo() {
  return (
    <div className="flex flex-wrap gap-4 mb-4">
      <BirdCardP13
        label="WAIR · active"
        subLabel="rapid flap + 28 deg forward pitch"
        badge="P13.2"
        state={{ heading: 0, mapBearing: 0, speed: 0, navigating: false }}
        wairMode={true}
      />
      <BirdCardP13
        label="WAIR · idle (control)"
        subLabel="normal idle posture"
        badge="control"
        state={{ heading: 0, mapBearing: 0, speed: 0, navigating: false }}
        wairMode={false}
      />
      <BirdCardP13
        label="WAIR · mid zoom"
        subLabel="detail visible at zoom=12"
        badge="P13.2"
        state={{ heading: 0, mapBearing: 0, speed: 0, navigating: false }}
        mapZoom={12}
        wairMode={true}
...
/* ── Phase 13: Soaring demo ──────────────────────────────────────────────── */
function SoaringDemo() {
  return (
    <div className="flex flex-wrap gap-4 mb-4">
      <BirdCardP13
        label="Soaring · dive phase"
        subLabel="4.2s cycle — nose-down dive"
        badge="P13.3"
        state={{ heading: 0, mapBearing: 0, speed: 55, navigating: true }}
        soaring={true}
      />
      <BirdCardP13
        label="Soaring · high zoom"
        subLabel="full plumage detail"
        badge="P13.3"
        state={{ heading: 45, mapBearing: 0, speed: 55, navigating: true }}
        mapZoom={17}
        soaring={true}
      ...
      />
      <BirdCardP13
        label="Gliding (control)"
        subLabel="normal glide — no soar cycle"
        badge="control"
        state={{ heading: 0, mapBearing: 0, speed: 55, navigating: true }}
        soaring={false}
      />
...
/* ── Phase 13: Mating display demo ───────────────────────────────────────── */
function MatingDisplayDemo() {
  return (
    <div className="flex flex-wrap gap-4 mb-4">
      <BirdCardP13
        label="Mating display"
        subLabel="1.6s — pivot + wing fan + head bow"
        badge="P13.4"
        state={{ heading: 0, mapBearing: 0, speed: 0, navigating: false }}
        matingDisplay={true}
        size={56}
      />
      <BirdCardP13
        label="Mating · street zoom"
        subLabel="crown flash visible"
        badge="P13.4"
        state={{ heading: 0, mapBearing: 0, speed: 0, navigating: false }}
        mapZoom={17}
        matingDisplay={true}
      ...
      <BirdCardP13
        label="Idle (control)"
        subLabel="normal idle for comparison"
        badge="control"
        state={{ heading: 0, mapBearing: 0, speed: 0, navigating: false }}
        matingDisplay={false}
      />
...
/* ── Phase 13: Hover wrist + knee articulation demo ──────────────────────── */
function HoverAndKneeDemo() {
  return (
    <div className="flex flex-wrap gap-4 mb-4">
      <BirdCardP13
        label="Hover · wrist rotation"
        subLabel="680ms -- lift on both strokes"
        badge="P13.5"
        state={{ heading: 0, mapBearing: 0, speed: 0, navigating: false }}
      />
      <BirdCardP13
        label="Knee articulation"
        subLabel="walking stride -- alt L/R"
        badge="P13.6"
        state={{ heading: 0, mapBearing: 0, speed: 1.4, navigating: true }}
        mapZoom={17}
      />
      <BirdCardP13
        label="Knee · running"
        subLabel="faster alt stride 0.45s"
        badge="P13.6"
        state={{ heading: 0, mapBearing: 0, speed: 5, navigating: true }}
        mapZoom={17}
      />
      <BirdCardP13
        label="Knee · street zoom"
        subLabel="joint dots visible"
        badge="P13.6"
        state={{ heading: 0, mapBearing: 0, speed: 1.4, navigating: true }}
        mapZoom={18}
        size={60}
...
/* ── Phase 13: Full body gaze chain demo ──────────────────────────────────── */
function GazeBodyChainDemo() {
  const [gazeIdx, setGazeIdx] = useState(0);
  const gazes: Array<{ heading: number | null; label: string; subLabel: string }> = [
    { heading: null, label: "Gaze -- forward",  subLabel: "neutral: all segments centered" },
    { heading: null, label: "Gaze -- left",     subLabel: "iris+head+neck+body+tail+wing curl" },
    { heading: null, label: "Gaze -- right",    subLabel: "iris+head+neck+body+tail+wing curl" },
    { heading: null, label: "Gaze -- upleft",   subLabel: "diagonal chain response" },
    { heading: null, label: "Gaze -- upright",  subLabel: "diagonal chain response" },
  ];

  useEffect(() => {
    const id = setInterval(() => setGazeIdx(i => (i + 1) % gazes.length), 2800);
    return () => clearInterval(id);
  }, [gazes.length]);

  const directions = ["forward", "left", "right", "upleft", "upright"] as const;
  const turnDir = directions[gazeIdx] === "left" || directions[gazeIdx] === "upleft"
    ? "left"
    : directions[gazeIdx] === "right" || directions[gazeIdx] === "upright"
      ? "right"
      : null;
  ...
  return (
    <div className="flex flex-wrap gap-4 mb-4">
      ...
      >
        <div style={{ width: 80, height: 80, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <SankofaBird
            ...
            size={56}
            mapZoom={17}
            upcomingTurnDirection={turnDir}
          />
        </div>
        <div className="text-center">
          <p className="text-xs font-semibold text-white">{gazes[gazeIdx].label}</p>
          <p className="text-[10px] mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>{gazes[gazeIdx].subLabel}</p>
          <span className="inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] font-semibold"
            style={{ background: "rgba(80,200,120,0.15)", color: "rgba(80,200,120,0.9)", border: "1px solid rgba(80,200,120,0.25)" }}>
            P13.8 cycling
          </span>
      ...
      {(["left", "right"] as const).map(dir => (
        <BirdCard
          key={dir}
          label={`Full chain -- gaze ${dir}`}
          subLabel="iris+head+neck+body+tail+shoulder"
          badge="P13.8"
          state={{ heading: 0, mapBearing: 0, speed: 0, navigating: false }}
          mapZoom={17}
          size={52}
...
/* ── Phase 13: All aerodynamic states panel ──────────────────────────────── */
function AerodynamicsPhase13Panel() {
  return (
    <div className="flex flex-wrap gap-4 mb-4">
      <BirdCardP13
        label="Flap · oval stroke"
        subLabel="running -- P13.1 figure-8"
        badge="P13.1"
        state={{ heading: 90, mapBearing: 0, speed: 5, navigating: true }}
      />
      <BirdCardP13
        label="WAIR"
        subLabel="wing+leg climbing mode"
        badge="P13.2"
        state={{ heading: 90, mapBearing: 0, speed: 0, navigating: false }}
        wairMode={true}
      />
      ...
      <BirdCardP13
        label="Hover · wrist"
        subLabel="680ms wrist-rotation"
        badge="P13.5"
        state={{ heading: 90, mapBearing: 0, speed: 0, navigating: false }}
      />
      <BirdCardP13
        label="Knee stride"
        subLabel="alt L/R cadence"
        badge="P13.6"
        state={{ heading: 90, mapBearing: 0, speed: 1.4, navigating: true }}
      />
      <BirdCardP13
        label="Murmur wave"
        subLabel="driving -- iridescent ripple"
        badge="P13.7"
        state={{ heading: 90, mapBearing: 0, speed: 14, navigating: true }}
      />
    </div>
...
/* ── Phase 20: 360° Multi-View Sprite Cross-Fade Demo ────────────────────
   Verifies:
   • FrontView renders at N headings (0°), BackView at S (180°)
   • Side sprite remains for E/W headings with correct scaleX flip
   • 45° linear cross-fades at NE/SE/SW/NW diagonals
   • Walking legs animate at all three views (data-speed="walking")
   • Live sweep confirms continuous opacity transition with no snap         */
function MultiView360Demo() {
  ...
  // 8 compass headings: N, NE, E, SE, S, SW, W, NW
  const HEADINGS = [
    { deg: 0,   label: "N  0°",   sprite: "Front",   bg: "rgba(0,180,255,0.12)" },
    { deg: 45,  label: "NE 45°",  sprite: "↔ blend",  bg: "rgba(100,100,100,0.08)" },
    { deg: 90,  label: "E  90°",  sprite: "Side →",  bg: "rgba(255,255,255,0.06)" },
    { deg: 135, label: "SE 135°", sprite: "↔ blend",  bg: "rgba(100,100,100,0.08)" },
    { deg: 180, label: "S 180°",  sprite: "Back",    bg: "rgba(160,80,255,0.12)" },
    { deg: 225, label: "SW 225°", sprite: "↔ blend",  bg: "rgba(100,100,100,0.08)" },
    { deg: 270, label: "W 270°",  sprite: "Side ←",  bg: "rgba(255,255,255,0.06)" },
    { deg: 315, label: "NW 315°", sprite: "↔ blend",  bg: "rgba(100,100,100,0.08)" },
  ];

  return (
    <div className="space-y-5 mb-4">
      {/* Static: 8 headings with labels */}
      <div>
        <p className="text-xs mb-3" style={{ color: "rgba(255,255,255,0.4)" }}>
          8 compass headings — walking speed (1.4 m/s) so legs animate. Pure
          <span style={{ color: "rgba(0,180,255,0.9)" }}> front</span> /
          <span style={{ color: "rgba(255,255,255,0.7)" }}> side</span> /
          <span style={{ color: "rgba(160,80,255,0.9)" }}> back</span> at
          cardinal points; cross-fades at diagonals.
        </p>
        <div className="flex flex-wrap gap-3">
          {HEADINGS.map(({ deg, label, sprite, bg }) => (
            <div
              key={deg}
              className="flex flex-col items-center gap-2 rounded-xl p-3"
              style={{ background: bg, border: "1px solid rgba(255,255,255,0.09)", minWidth: 88 }}
            >
              <SankofaBird
                heading={deg}
                mapBearing={0}
                ...
                mapZoom={15}
              />
              <span className="text-xs font-mono" style={{ color: "rgba(0,212,255,0.8)" }}>{label}</span>
              <span className="text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>{sprite}</span>
            </div>
          ))}
        </div>
      </div>

      {/* Live sweep — continuous rotation showing smooth cross-fades */}
      <div className="flex items-center gap-6">
        ...
        >
          <SankofaBird
            ...
            mapZoom={15}
          />
          <span className="text-xs font-mono" style={{ color: "rgba(0,212,255,0.9)" }}>
            {Math.round(sweepDeg)}°
          </span>
          <span className="text-xs" style={{ color: "rgba(255,255,255,0.35)" }}>Live sweep</span>
        </div>
        <div className="space-y-2">
          <p className="text-xs" style={{ color: "rgba(255,255,255,0.5)" }}>
            Continuous 360° rotation at 21°/s (running tier).
            Cross-fades should be smooth — no pops or snaps.
          </p>
          <button
            onClick={() => setSweeping(s => !s)}
            className="text-xs rounded-lg px-4 py-2 font-medium"
            style={{
              background: sweeping ? "rgba(0,212,255,0.18)" : "rgba(255,255,255,0.06)",
              border: sweeping ? "1px solid rgba(0,212,255,0.5)" : "1px solid rgba(255,255,255,0.12)",
              color: sweeping ? "rgba(0,212,255,1)" : "rgba(255,255,255,0.7)",
              cursor: "pointer",
            }}
          >
            {sweeping ? "⏸ Pause sweep" : "▶ Start sweep"}
          </button>
          <p className="text-xs mt-1" style={{ color: "rgba(255,255,255,0.3)" }}>
            Front sprite: <span style={{ color: "rgba(0,180,255,0.8)" }}>N±22.5°</span>
            {" · "}Back sprite: <span style={{ color: "rgba(160,80,255,0.8)" }}>S±22.5°</span>
            {" · "}45° cross-fade zones at diagonals
          </p>
        </div>

        {/* Walking legs at N / E / S / W */}
        <div className="flex gap-2 ml-4">
          {[
            { deg: 0,   label: "N walk" },
            { deg: 90,  label: "E walk" },
            { deg: 180, label: "S walk" },
            { deg: 270, label: "W walk" },
          ].map(({ deg, label }) => (
...
/* ══════════════════════════════════════════════════════════════════════════
   NEW SECTIONS — Real-time gaze, activity controls, solar simulation,
   regression checklist (added July 2026)
   ══════════════════════════════════════════════════════════════════════════ */

/* ── Real-time Gaze Control Demo ──────────────────────────────────────────
   Shows all 9 gaze states simultaneously (8 directions + forward) plus a
   live cycling demo that auto-advances every 1.5 s through every direction.
   This lets the tester visually verify that every CSS rule fires correctly. */
function GazeRealTimeDemo() {
  const ALL_GAZES: Array<{
    label: string;
    subLabel: string;
    upcomingTurnDirection?: "left" | "right" | null;
    approaching?: boolean;
    ...
    { label: "forward",   subLabel: "neutral default",           navigating: false },
    { label: "upleft",    subLabel: "upcoming left turn",        upcomingTurnDirection: "left",  navigating: true },
    { label: "upright",   subLabel: "upcoming right turn",       upcomingTurnDirection: "right", navigating: true },
    { label: "up",        subLabel: "gliding / thermal scan",    navigating: true },
    { label: "down",      subLabel: "approaching destination",   approaching: true },
    { label: "left",      subLabel: "hard bank left",            navigating: true },
    { label: "right",     subLabel: "notification / bank right", newNotification: true },
    { label: "downleft",  subLabel: "idle saccade drift",        navigating: false },
    { label: "downright", subLabel: "idle saccade drift",        navigating: false },
  ];

  // Live cycling demo — steps through all 9 states
  const [liveIdx, setLiveIdx] = useState(0);
  useEffect(() => {
    const id = setInterval(() => setLiveIdx(i => (i + 1) % ALL_GAZES.length), 1500);
    return () => clearInterval(id);
  }, [ALL_GAZES.length]);

  // Banking demo — continuous heading sweep to trigger bankDeg gaze
  const [bankHeading, setBankHeading] = useState(90);
  useEffect(() => {
    let h = 90, dir = 1, ticks = 0;
    const id = setInterval(() => {
      h = (h + dir * 3 + 360) % 360;
      ticks++;
      if (ticks >= 16) { dir = -dir; ticks = 0; }
      setBankHeading(Math.round(h));
    }, 60);
    return () => clearInterval(id);
  }, []);

  const live = ALL_GAZES[liveIdx];
    ...
    <div className="mb-8">
      ...
      <div className="flex items-start gap-6 mb-6">
        <div className="flex flex-col items-center gap-2 rounded-xl p-4"
          style={{ background: "rgba(0,212,255,0.06)", border: "1px solid rgba(0,212,255,0.25)", minWidth: 160 }}>
          <div style={{ width: 88, height: 88, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <SankofaBird
              heading={0} mapBearing={0}
              speed={live.navigating ? 5 : 0}
              navigating={live.navigating ?? false}
              size={60} mapZoom={17}
              upcomingTurnDirection={live.upcomingTurnDirection ?? null}
              approaching={live.approaching ?? false}
              newNotification={live.newNotification ?? false}
            />
          </div>
          <div className="text-center">
            <p className="text-sm font-bold text-white">Live cycling</p>
            <p className="text-[11px] font-mono mt-0.5" style={{ color: "rgba(0,212,255,0.9)" }}>
              gaze: {live.label}
            </p>
            <p className="text-[10px] mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>{live.subLabel}</p>
          </div>
        </div>

        {/* Bank-responsive gaze — navigating=true (nav screen) */}
        <div className="flex flex-col items-center gap-2 rounded-xl p-4"
          style={{ background: "rgba(0,212,255,0.04)", border: "1px solid rgba(0,212,255,0.15)", minWidth: 150 }}>
          <div style={{ width: 88, height: 88, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <SankofaBird
              heading={bankHeading} mapBearing={0}
              speed={12} navigating={true}
              size={60} mapZoom={17}
            />
          </div>
          <div className="text-center">
            <p className="text-sm font-bold text-white">Bank-gaze (nav)</p>
            <p className="text-[10px] mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>
              navigating=<span style={{ color: "rgba(80,200,120,0.9)" }}>true</span> · hard bank → left/right
            </p>
            <p className="text-[10px] font-mono mt-0.5" style={{ color: "rgba(0,212,255,0.7)" }}>
              heading: {bankHeading}°
            </p>
          </div>
        </div>

        {/* Bank-responsive gaze — navigating=false (map screen). This card proves the
            fix: bankDeg is now ALWAYS passed to computeGazeVector regardless of navigating
            flag. Before the fix, this bird would show no bank-driven gaze on the map screen. */}
        <div className="flex flex-col items-center gap-2 rounded-xl p-4"
          style={{ background: "rgba(0,212,255,0.04)", border: "1px solid rgba(255,165,0,0.35)", minWidth: 150 }}>
          <div style={{ width: 88, height: 88, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <SankofaBird
              heading={bankHeading} mapBearing={0}
              speed={12} navigating={false}
              size={60} mapZoom={17}
            />
          </div>
          <div className="text-center">
            <p className="text-sm font-bold text-white">Bank-gaze (map)</p>
            <p className="text-[10px] mt-0.5" style={{ color: "rgba(255,165,0,0.8)" }}>
              navigating=<span style={{ color: "rgba(255,165,0,0.9)" }}>false</span> · should still gaze
            </p>
            <p className="text-[10px] font-mono mt-0.5" style={{ color: "rgba(0,212,255,0.7)" }}>
              heading: {bankHeading}°
            </p>
            <span className="inline-block mt-1 px-1.5 py-0.5 rounded text-[9px] font-bold"
              style={{ background: "rgba(255,165,0,0.15)", color: "rgba(255,165,0,0.9)", border: "1px solid rgba(255,165,0,0.3)" }}>
              regression guard
            </span>
          </div>
        </div>

        <div className="flex flex-col gap-1.5 pt-2 text-xs max-w-[200px]" style={{ color: "rgba(255,255,255,0.35)" }}>
          <p className="font-semibold" style={{ color: "rgba(0,212,255,0.7)" }}>Gaze priority chain:</p>
          <p>1. approaching → down</p>
          <p>2. upcoming turn → upleft/upright</p>
          <p>3. gliding → up</p>
          <p>4. notification → right</p>
          <p>4.5. hard bank (&gt;10°) → left/right</p>
          <p>5. helping → forward</p>
          <p>6. idle saccade → 8 directions</p>
        </div>
      </div>

      {/* All 9 states simultaneously */}
      <div className="flex flex-wrap gap-3">
        {ALL_GAZES.map(g => (
          <BirdCard
            key={g.label}
            ...
            state={{
              heading: 0, mapBearing: 0,
              speed: g.navigating ? 5 : 0,
              navigating: g.navigating ?? false,
              newNotification: g.newNotification,
            }}
            mapZoom={17} size={48}
            upcomingTurnDirection={g.upcomingTurnDirection ?? null}
            approaching={g.approaching ?? false}
          />
        ))}
      </div>
    </div>
...
/* ── Activity-Level Controls Demo ─────────────────────────────────────────
   Interactive slider for activityLevel (0-1) driving blink period and
   crown alertness. Shows 4 preset tiers + a freely adjustable control bird.
   Designed specifically for verifying the blink=crown animations work visually. */
function ActivityLevelControlDemo() {
  const [activityLevel, setActivityLevel] = useState(0.5);
  const [skyTier, setSkyTier] = useState<"day" | "golden" | "twilight" | "night">("day");
  ...
  const blinkMs =
    tier === "peak" ? 3500 : tier === "busy" ? 5000 : tier === "quiet" ? 9000 : 7000;

  const TIERS: Array<{ label: string; level: number; color: string }> = [
    { label: "quiet",  level: 0.05, color: "rgba(100,180,255,0.8)" },
    { label: "normal", level: 0.40, color: "rgba(0,212,255,0.8)" },
    { label: "busy",   level: 0.70, color: "rgba(255,200,60,0.9)" },
    { label: "peak",   level: 0.95, color: "rgba(255,80,80,0.9)" },
  ];

  return (
    <div className="mb-8">
      {/* Controls */}
      <div className="flex flex-wrap items-center gap-6 mb-5 p-4 rounded-xl"
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <span className="text-base font-black font-mono w-12" style={{ color: "rgba(0,212,255,0.9)" }}>
              {(activityLevel * 100).toFixed(0)}%
            </span>
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full"
              style={{
                background: tier === "peak" ? "rgba(255,80,80,0.15)" : tier === "busy" ? "rgba(255,200,60,0.15)" : tier === "quiet" ? "rgba(100,180,255,0.15)" : "rgba(0,212,255,0.12)",
                color: tier === "peak" ? "rgba(255,100,100,1)" : tier === "busy" ? "rgba(255,210,80,1)" : tier === "quiet" ? "rgba(140,200,255,1)" : "rgba(0,212,255,1)",
          ...
          <p className="text-[10px]" style={{ color: "rgba(255,255,255,0.3)" }}>
            blink period: {(blinkMs / 1000).toFixed(1)} s · crown sway: {
              tier === "peak" ? "1.1s rapid" : tier === "busy" ? "2.4s fast" : tier === "quiet" ? "5.2s slow" : "3.6s normal"
            }
          </p>
        </div>

        <div className="flex flex-col gap-1.5">
          <span className="text-xs font-semibold" style={{ color: "rgba(255,255,255,0.55)" }}>Sky tier</span>
          <div className="flex gap-2">
            {(["day", "golden", "twilight", "night"] as const).map(t => (
              <button key={t} onClick={() => setSkyTier(t)}
                className="px-2 py-1 rounded-lg text-[11px] font-semibold transition-all"
                style={{
                  background: skyTier === t ? "rgba(0,212,255,0.2)" : "rgba(255,255,255,0.05)",
                  border: skyTier === t ? "1px solid rgba(0,212,255,0.5)" : "1px solid rgba(255,255,255,0.1)",
                  color: t === "night" ? "rgba(140,180,255,1)" : t === "twilight" ? "rgba(180,140,255,1)" : t === "golden" ? "rgba(255,200,80,1)" : "rgba(160,220,255,1)",
                }}>
                {t === "night" ? "🌙" : t === "twilight" ? "🌆" : t === "golden" ? "🌇" : "☀️"} {t}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-4 mb-4">
        {TIERS.map(t => (
          <div key={t.label}
            ...
            style={{
              background: "rgba(255,255,255,0.03)",
              border: `1px solid ${t.color.replace("0.8)", "0.3)")}`,
              minWidth: 120,
            }}
            onClick={() => setActivityLevel(t.level)}
          >
            <div style={{ width: 68, height: 68, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <SankofaBird
                heading={0} mapBearing={0} speed={0} navigating={false}
                size={50} mapZoom={17}
                activityLevel={t.level} skyTier={skyTier}
              />
            </div>
            <div className="text-center">
              <p className="text-xs font-bold" style={{ color: t.color }}>{t.label}</p>
              <p className="text-[10px]" style={{ color: "rgba(255,255,255,0.35)" }}>{(t.level * 100).toFixed(0)}% activity</p>
            </div>
          </div>
        ))}

        {/* Live-controlled bird */}
        <div className="flex flex-col items-center gap-2 rounded-xl p-3"
          style={{ background: "rgba(0,212,255,0.06)", border: "2px solid rgba(0,212,255,0.3)", minWidth: 130 }}>
          <div style={{ width: 80, height: 80, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <SankofaBird
              heading={0} mapBearing={0} speed={0} navigating={false}
              size={58} mapZoom={17}
              activityLevel={activityLevel} skyTier={skyTier}
            />
          </div>
          <div className="text-center">
            <p className="text-xs font-bold" style={{ color: "rgba(0,212,255,1)" }}>Live control</p>
            <p className="text-[10px]" style={{ color: "rgba(255,255,255,0.45)" }}>
              Drag slider above
            </p>
            <span className="inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] font-semibold"
              style={{ background: "rgba(0,212,255,0.15)", color: "rgba(0,212,255,0.9)", border: "1px solid rgba(0,212,255,0.3)" }}>
              {tier}
            </span>
          </div>
        </div>
      ...
      {/* Flying bird with activity effect */}
      <div className="flex flex-wrap gap-4">
        {TIERS.map(t => (
          <BirdCard
            key={`fly-${t.label}`}
            label={`Flying · ${t.label}`}
            subLabel={`${(t.level * 100).toFixed(0)}% — blink + crown`}
            badge={t.label}
            state={{ heading: 45, mapBearing: 0, speed: 8, navigating: true }}
            mapZoom={17} size={48}
            activityLevel={t.level}
            skyTier={skyTier}
          />
        ))}
      </div>
...
/* ── Solar Sky-Tier Simulation Demo ────────────────────────────────────────
   Interactive sun-elevation slider (−90° → +90°) that derives the sky tier
   using the same thresholds as useSolarTier() in the live app.
   Lets testers verify sunset transitions without waiting for real sunset. */
function SolarSimDemo() {
  const [elevation, setElevation] = useState(35); // degrees above horizon

  const skyTier: "day" | "golden" | "twilight" | "night" =
    elevation > 10 ? "day" :
    elevation > 0  ? "golden" :
    elevation > -6 ? "twilight" : "night";

  const tierColor =
    skyTier === "night" ? "rgba(100,140,255,0.9)" :
    skyTier === "twilight" ? "rgba(180,120,255,0.9)" :
    skyTier === "golden" ? "rgba(255,200,60,0.9)" : "rgba(160,220,255,0.9)";

  const tierEmoji = skyTier === "night" ? "🌙" : skyTier === "twilight" ? "🌆" : skyTier === "golden" ? "🌇" : "☀️";

  const bgColor =
    skyTier === "night" ? "rgba(8,12,35,0.95)" :
    skyTier === "twilight" ? "rgba(18,12,40,0.85)" :
    skyTier === "golden" ? "rgba(40,22,8,0.85)" : "rgba(20,30,45,0.4)";

  return (
    <div className="mb-8">
      <div className="flex flex-wrap items-center gap-8 mb-5 p-4 rounded-xl"
        style={{ background: bgColor, border: `1px solid ${tierColor.replace("0.9)", "0.25)")}`, transition: "all 1.2s ease" }}>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-mono w-14 text-right" style={{ color: "rgba(255,255,255,0.35)" }}>−90° (nadir)</span>
            <input
              type="range" min={-90} max={90} step={1}
              value={elevation}
              onChange={e => setElevation(Number(e.target.value))}
              style={{ width: 220, accentColor: tierColor.replace("0.9)", "1)") }}
            />
            <span className="text-[10px] font-mono w-16" style={{ color: "rgba(255,255,255,0.35)" }}>+90° (zenith)</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-sm font-black font-mono" style={{ color: tierColor }}>{elevation > 0 ? "+" : ""}{elevation}°</span>
            <span className="text-base font-black px-3 py-1 rounded-lg" style={{ background: tierColor.replace("0.9)", "0.15)"), color: tierColor, border: `1px solid ${tierColor.replace("0.9)", "0.3)")}` }}>
              {tierEmoji} {skyTier}
            </span>
          </div>
          <div className="text-[10px] font-mono" style={{ color: "rgba(255,255,255,0.28)" }}>
            Thresholds: night &lt; −6° · twilight −6°→0° · golden 0°→10° · day &gt; 10°
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="text-[10px] text-center font-mono" style={{ color: tierColor }}>
            skyTier=&quot;{skyTier}&quot;
          </p>
        </div>
      </div>

      {/* All 4 tiers side by side for comparison */}
      <div className="flex flex-wrap gap-4">
        {(["day", "golden", "twilight", "night"] as const).map(t => (
          <div key={t}
            className="flex flex-col items-center gap-2 rounded-xl p-3 cursor-pointer"
            style={{
              background: t === "night" ? "rgba(8,12,35,0.9)" : t === "twilight" ? "rgba(15,10,35,0.8)" : t === "golden" ? "rgba(35,18,5,0.8)" : "rgba(255,255,255,0.03)",
              border: skyTier === t ? `2px solid ${tierColor}` : "1px solid rgba(255,255,255,0.08)",
              minWidth: 120, transition: "border 0.4s",
            }}
            onClick={() => setElevation(t === "day" ? 40 : t === "golden" ? 6 : t === "twilight" ? -3 : -20)}
          >
            <div style={{ width: 72, height: 72, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <SankofaBird heading={0} mapBearing={0} speed={0} navigating={false} size={52} mapZoom={16} skyTier={t} />
            </div>
            <p className="text-xs font-bold text-center" style={{
              color: t === "night" ? "rgba(100,140,255,0.9)" : t === "twilight" ? "rgba(180,120,255,0.9)" : t === "golden" ? "rgba(255,200,60,0.9)" : "rgba(160,220,255,0.9)"
            }}>
              {t === "night" ? "🌙 night" : t === "twilight" ? "🌆 twilight" : t === "golden" ? "🌇 golden" : "☀️ day"}
            </p>
            <p className="text-[10px] text-center" style={{ color: "rgba(255,255,255,0.3)" }}>
              {t === "night" ? "sun &lt; −6°" : t === "twilight" ? "−6° to 0°" : t === "golden" ? "0° to 10°" : "sun &gt; 10°"}
            </p>
          </div>
        ))}
      </div>
...
/* ── Phase 20 SME Physics Demo ──────────────────────────────────────────────
   Shows live gauges for all 8 SME physics channels (written each rAF frame
   to CSS vars) and demonstrates the new features:
     • Dynamic neck bezier: neck visually bends toward head direction
     • Eye tracking: eyes follow direction of travel, not heading lag
   Three birds: idle, cruising, and hard-banking — so all channels are active. */
function Phase20SmePhysicsDemo() {
  const [heading, setHeading] = useState(90);
  const [speed, setSpeed]     = useState(14);
  const [auto, setAuto]       = useState(true);
  const rafRef = useRef<number | null>(null);

  // Auto-pilot: sweep heading continuously so bank/neck/eye all animate
  useEffect(() => {
  ...
  const scenarios: Array<{ label: string; spd: number; hdg: number | null; nav: boolean }> = [
    { label: "Idle (eye → center)",        spd: 0,  hdg: null, nav: false },
    { label: "Cruise 14 m/s (eye → fwd)",  spd: 14, hdg: heading, nav: true  },
    { label: "Glide 55 m/s (neck bends)",  spd: 55, hdg: heading, nav: true  },
    { label: "Turn — neck + eye active",   spd: 10, hdg: heading, nav: true  },
  ];

  return (
    <div className="rounded-xl p-5 mb-6" style={{ background: "rgba(0,10,20,0.6)", border: "1px solid rgba(0,212,255,0.15)" }}>
      {/* Controls */}
      <div className="flex flex-wrap items-center gap-4 mb-5">
        <div className="flex flex-col gap-1">
          <label className="text-[10px] uppercase tracking-widest" style={{ color: "rgba(0,212,255,0.6)" }}>
            Heading (degrees)
          </label>
          <div className="flex items-center gap-2">
            <input
              ...
              value={heading}
              onChange={e => { setAuto(false); setHeading(Number(e.target.value)); }}
              style={{ width: 160, accentColor: "#00D4FF" }}
            />
            <span className="text-sm font-mono font-bold" style={{ color: "#00D4FF", minWidth: 40 }}>{heading}°</span>
          </div>
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-[10px] uppercase tracking-widest" style={{ color: "rgba(0,212,255,0.6)" }}>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-sm font-mono font-bold" style={{ color: "#00D4FF", minWidth: 40 }}>{speed} m/s</span>
          </div>
        </div>
        <button
          onClick={() => setAuto(a => !a)}
          ...
          style={{
            background: auto ? "rgba(0,212,255,0.15)" : "rgba(255,255,255,0.06)",
            border: `1px solid ${auto ? "rgba(0,212,255,0.4)" : "rgba(255,255,255,0.12)"}`,
            color: auto ? "#00D4FF" : "rgba(255,255,255,0.5)",
          }}
        >
          {auto ? "⟳ Auto" : "⏸ Manual"}
        </button>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        {[
          { title: "Dynamic neck bezier", desc: "Neck path d= recomputed each rAF frame. Curves toward head direction. No Rive .riv needed.", color: "#00D4FF" },
          { title: "Eye → travel direction", desc: "Eyes track where the bird is flying (−X = toward beak = forward), not heading lag.", color: "#4ADE80" },
          { title: "Wind heading blend", desc: "effectiveHeading = lerp(heading, windHeading, strength×0.3) before kinematic chain.", color: "#FACC15" },
          { title: "Flutter package", desc: "packages/sankofa_motion_engine/ — standalone Dart port with 8 motion_solver_test.dart tests.", color: "#C084FC" },
        ].map(f => (
          <div key={f.title} className="rounded-lg p-3 flex-1 min-w-[200px]" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${f.color}22` }}>
      ...
      <div className="flex flex-wrap gap-6">
        {scenarios.map(s => (
          <div key={s.label} className="flex flex-col items-center gap-2">
            ...
            <p className="text-[10px] text-center max-w-[100px] leading-tight" style={{ color: "rgba(255,255,255,0.45)" }}>
              {s.label}
            </p>
          </div>
        ))}
      ...
      {/* SME CSS var legend */}
      <div className="mt-5 pt-4 border-t" style={{ borderColor: "rgba(0,212,255,0.1)" }}>
        <p className="text-[10px] mb-2 uppercase tracking-widest" style={{ color: "rgba(0,212,255,0.5)" }}>SME CSS var channels (written each rAF @ 60fps)</p>
        <div className="flex flex-wrap gap-x-6 gap-y-1">
          {[
            "--sme-head-deg", "--sme-neck-upper-deg", "--sme-neck-lower-deg",
            "--sme-body-roll-deg", "--sme-tail-deg",
            "--sme-lwing-upper-deg", "--sme-rwing-upper-deg",
            "--sme-eye-x", "--sme-eye-y",
            "--sme-flap-phase", "--sme-flap-amplitude",
            "--sme-notification-pulse", "--sme-wind-strength",
            "--mixer-neck-curve-deg", "--mixer-head-lead-deg",
            "--mixer-bank-deg", "--mixer-heading-deg",
          ].map(v => (
            <code key={v} className="text-[9px] font-mono" style={{ color: "rgba(0,212,255,0.4)" }}>{v}</code>
          ))}
        </div>
      </div>
    </div>
...
/* ── Regression Checklist Panel ──────────────────────────────────────────────
   Auto-cycles through every critical animation state every 2 s and renders
   them all simultaneously. The tester can scan visually for broken states.
   All birds run simultaneously — no interaction needed. */
function RegressionChecklistPanel() {
  ...
  const CHECKS: Array<{
    ...
    subLabel: string;
    props: {
      heading?: number | null; mapBearing?: number; speed?: number;
      navigating?: boolean; celebrating?: boolean; newNotification?: boolean;
      accepted?: boolean; donated?: boolean; nearbyUser?: boolean;
      approaching?: boolean; upcomingTurnDirection?: "left" | "right" | null;
      isHelping?: boolean; batterySaver?: boolean; mapZoom?: number;
      skyTier?: "day" | "golden" | "twilight" | "night"; activityLevel?: number;
      navLodOverride?: 0 | 1 | 2;
    };
  }> = [
    { id: "idle",       label: "Idle",          subLabel: "float + blink",   props: { heading: null, speed: 0, navigating: false } },
    { id: "walking",    label: "Walk 1.4 m/s",  subLabel: "slow flap",       props: { heading: 0, speed: 1.4, navigating: true } },
    { id: "driving",    label: "Drive 14 m/s",  subLabel: "fast flap + lean", props: { heading: 0, speed: 14, navigating: true } },
    { id: "gliding",    label: "Glide 55 m/s",  subLabel: "4s period",       props: { heading: 0, speed: 55, navigating: true } },
    { id: "banking",    label: "Banking",       subLabel: "live delta",       props: { heading: bankHeading, speed: 12, navigating: true } },
    { id: "celebrate",  label: "Celebrate",     subLabel: "teal burst",      props: { celebrating: true, heading: null, speed: 0, navigating: false } },
    { id: "notif",      label: "Notification",  subLabel: "saccade burst",   props: { newNotification: true, heading: null, speed: 0, navigating: false } },
    { id: "helping",    label: "Helping",       subLabel: "gold crane",      props: { isHelping: true, heading: 0, speed: 8, navigating: true } },
    { id: "turn-left",  label: "Turn left",     subLabel: "upleft gaze",     props: { upcomingTurnDirection: "left", heading: bankHeading, speed: 10, navigating: true } },
    { id: "approach",   label: "Approaching",   subLabel: "slow + down gaze", props: { approaching: true, heading: 0, speed: 4, navigating: true } },
    { id: "night",      label: "Night",         subLabel: "P10 plumage",     props: { skyTier: "night", heading: null, speed: 0, navigating: false } },
    { id: "golden",     label: "Golden hr",     subLabel: "amber wash",      props: { skyTier: "golden", heading: null, speed: 0, navigating: false } },
    { id: "battery",    label: "Battery LOD3",  subLabel: "silhouette only", props: { batterySaver: true, heading: 0, speed: 5, navigating: true } },
    { id: "lod2",       label: "Nav LOD2",      subLabel: "30min+ session",  props: { navLodOverride: 2, heading: 0, speed: 8, navigating: true } },
    { id: "low-zoom",   label: "Low zoom",      subLabel: "city scale z=8",  props: { heading: 0, speed: 5, navigating: true, mapZoom: 8 } },
    { id: "peak",       label: "Peak activity", subLabel: "rapid blink",     props: { activityLevel: 0.95, heading: null, speed: 0, navigating: false } },
    { id: "donated",    label: "Donated",       subLabel: "gold sparkle",    props: { donated: true, heading: null, speed: 0, navigating: false } },
    { id: "accepted",   label: "Accepted",      subLabel: "hop + stretch",   props: { accepted: true, heading: null, speed: 0, navigating: false } },
  ];

  return (
    <div className="mb-8">
      <p className="text-xs mb-4" style={{ color: "rgba(255,255,255,0.35)" }}>
        All states rendered simultaneously. Banking cards use live heading delta so bank animation actually fires.
        Scan visually — any broken, frozen, or absent animation is a regression.
      </p>
      <div className="flex flex-wrap gap-3">
        {CHECKS.map(c => (
          <BirdCard
            ...
            badge="✓ check"
            state={{
              heading: c.props.heading !== undefined ? (c.props.heading ?? null) : null,
              mapBearing: c.props.mapBearing ?? 0,
              speed: c.props.speed ?? 0,
              navigating: c.props.navigating ?? false,
              celebrating: c.props.celebrating,
              newNotification: c.props.newNotification,
              accepted: c.props.accepted,
              donated: c.props.donated,
            }}
            mapZoom={c.props.mapZoom ?? 15}
            size={44}
            nearbyUser={c.props.nearbyUser}
            upcomingTurnDirection={c.props.upcomingTurnDirection ?? null}
            approaching={c.props.approaching ?? false}
            isHelping={c.props.isHelping ?? false}
            batterySaver={c.props.batterySaver ?? false}
            skyTier={c.props.skyTier}
            activityLevel={c.props.activityLevel ?? 0.4}
            navLodOverride={c.props.navLodOverride}
          />
        ))}
      </div>
    </div>
