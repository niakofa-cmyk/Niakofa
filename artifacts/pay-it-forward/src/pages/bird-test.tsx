  ...
  return (
    ...
    >
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        {/* ── Phase 12/13 — Real-time gaze + gap-closure ───────────────── */}
        <SectionLabel>Phase 12/13 — Real-time 8-direction gaze + gap-closure</SectionLabel>
        <p className="text-xs mb-3" style={{ color: "rgba(255,255,255,0.35)" }}>
          Analog CSS-var gaze (--gaze-x / --gaze-y) drives eye/iris/catchlight translation in real time.
          data-gaze rotates head and neck as a bone chain (head leads, neck lags 0.35s).
          The bird glances toward the NEXT turn BEFORE the map instruction fires.
          All 10 Phase-12 audit gaps closed: shadow gold on helping, body lean on upcoming turn,
          wing pre-extension, thermal lift, notification saccades, per-feather flight oscillations,
          pre-bank feather compression, helping low-zoom ring, speed-correlated crown sway.
        </p>
        <GazeDirectionDemo />
...
/* ══════════════════════════════════════════════════════════════════════════════
   PHASE 12/13 — Real-time gaze demo + gap-closure visual regression cards
   ══════════════════════════════════════════════════════════════════════════ */
function GazeDirectionDemo() {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setIdx(i => {
        const next = (i + 1) % dirs.length;
        setGazeDir(dirs[next]);
        return next;
      });
    }, 1800);
  ...
  // Simulate a bird approaching a turn — upcoming left then right, cycling
  const [upcomingTurn, setUpcomingTurn] = useState<"left" | "right" | null>(null);
  useEffect(() => {
    const seq: Array<"left" | "right" | null> = [null, "left", null, "right", null];
    let i = 0;
    const t = setInterval(() => {
      setUpcomingTurn(seq[i % seq.length]);
      i++;
    }, 2500);
    return () => clearInterval(t);
  }, []);
  ...
  return (
    <div className="flex flex-wrap gap-4">
      {/* Auto-cycling gaze — all 8 directions */}
      <div className="flex flex-col items-center gap-1">
        <SankofaBird heading={0} mapBearing={0} speed={8} navigating={true}
          upcomingTurnDirection={null} mapZoom={17} size={52} />
        <span className="text-[10px] text-center" style={{ color: "#7ee8fa" }}>
          Auto-gaze cycle<br /><b style={{ color: "#fff" }}>{gazeDir}</b>
        </span>
        <span className="text-[9px] mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>P12 analog CSS vars</span>
      </div>

      {/* Anticipatory look toward upcoming turn — fires BEFORE the instruction */}
      <div className="flex flex-col items-center gap-1">
        <SankofaBird heading={0} mapBearing={0} speed={12} navigating={true}
          upcomingTurnDirection={upcomingTurn} mapZoom={17} size={52} />
        <span className="text-[10px] text-center" style={{ color: "#7ee8fa" }}>
          Pre-turn glance<br /><b style={{ color: "#fff" }}>upcoming: {upcomingTurn ?? "none"}</b>
        </span>
        <span className="text-[9px] mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>P12 fires before instruction</span>
      </div>

      {/* Gaze left (left turn upcoming) */}
      <div className="flex flex-col items-center gap-1">
        <SankofaBird heading={0} mapBearing={0} speed={10} navigating={true}
          upcomingTurnDirection="left" mapZoom={17} size={52} />
        <span className="text-[10px] text-center" style={{ color: "#7ee8fa" }}>
          Gaze left<br /><b style={{ color: "#fff" }}>upcoming-turn=left</b>
        </span>
        <span className="text-[9px] mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>eye+iris+head+neck</span>
      </div>

      {/* Gaze right */}
      <div className="flex flex-col items-center gap-1">
        <SankofaBird heading={0} mapBearing={0} speed={10} navigating={true}
          upcomingTurnDirection="right" mapZoom={17} size={52} />
        <span className="text-[10px] text-center" style={{ color: "#7ee8fa" }}>
          Gaze right<br /><b style={{ color: "#fff" }}>upcoming-turn=right</b>
        </span>
        <span className="text-[9px] mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>bone-chain lag 0.35s</span>
      </div>

      {/* Gaze up — takeoff */}
      <div className="flex flex-col items-center gap-1">
        <SankofaBird heading={0} mapBearing={0} speed={0} navigating={false}
          mapZoom={17} size={52} />
        <span className="text-[10px] text-center" style={{ color: "#7ee8fa" }}>
          Perched · idle gaze<br /><b style={{ color: "#fff" }}>center / curiosity tilt</b>
        </span>
        <span className="text-[9px] mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>P7.3 curiosity scan</span>
      </div>

      {/* Shadow gold when helping */}
      <div className="flex flex-col items-center gap-1">
        <SankofaBird heading={45} mapBearing={0} speed={6} navigating={true}
          isHelping={true} mapZoom={14} size={52} />
        <span className="text-[10px] text-center" style={{ color: "#fbbf24" }}>
          Helping shadow<br /><b style={{ color: "#fff" }}>gold tint · G2</b>
        </span>
        <span className="text-[9px] mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>P13 shadow coloring</span>
      </div>

      {/* Helping low-zoom gold ring */}
      <div className="flex flex-col items-center gap-1">
        <SankofaBird heading={90} mapBearing={0} speed={8} navigating={true}
          isHelping={true} mapZoom={8} size={52} />
        <span className="text-[10px] text-center" style={{ color: "#fbbf24" }}>
          Helping · low zoom<br /><b style={{ color: "#fff" }}>gold ring · G8</b>
        </span>
        <span className="text-[9px] mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>P13 LOD3 helping rule</span>
      </div>

      {/* Thermal lift gliding */}
      <div className="flex flex-col items-center gap-1">
        <SankofaBird heading={270} mapBearing={0} speed={55} navigating={true}
          mapZoom={15} size={52} />
        <span className="text-[10px] text-center" style={{ color: "#7ee8fa" }}>
          Thermal lift<br /><b style={{ color: "#fff" }}>airplane glide · G4</b>
        </span>
        <span className="text-[9px] mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>P13 upward body float</span>
      </div>

      {/* Speed crown sway */}
      <div className="flex flex-col items-center gap-1">
        <SankofaBird heading={90} mapBearing={0} speed={55} navigating={true}
          mapZoom={17} size={52} />
        <span className="text-[10px] text-center" style={{ color: "#7ee8fa" }}>
          Crown sway speed<br /><b style={{ color: "#fff" }}>airplane=2.2s · G9</b>
        </span>
