...
import { ChevronLeft, Star, Heart, MapPin, Shield, CheckCircle2, Clock, Gift, MessageCircle, AlertTriangle, Languages, Award } from "lucide-react";
...
export default function HelperProfileScreen() {
  ...
  useEffect(() => {
    ...
    // Load recent completed requests by this helper (non-critical — section
    // is hidden when empty, so a fetch failure is a silent graceful degradation,
    // not a broken page; we still log so it's visible in devtools if debugging).
    fetch(`${base}/api/requests?helper_id=${helperId}&status=completed&limit=5`, { headers: authHeaders() })
      ...
      .catch((err) => console.warn("[helper-profile] recent helps fetch failed:", err));
  }, [helperId]);
  ...
  return (
    <div className="min-h-[100dvh] bg-background">
      ...
      <div className="px-4 pt-6 pb-safe pb-8 space-y-5 max-w-lg mx-auto">
        ...
        <div className={`rounded-2xl p-4 border ${noShowCount === 0 ? "bg-green-500/5 border-green-500/20" : noShowCount <= 2 ? "bg-yellow-500/5 border-yellow-500/20" : "bg-red-500/5 border-red-500/20"}`}>
          <div className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3 flex items-center gap-1.5">
            <Award className="w-3.5 h-3.5" /> Reliability
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="text-center">
              <div className={`text-2xl font-black ${noShowCount === 0 ? "text-green-400" : noShowCount <= 2 ? "text-yellow-400" : "text-red-400"}`}>
                {noShowCount}
              </div>
              <div className="text-[10px] text-muted-foreground mt-0.5 flex items-center justify-center gap-1">
                {noShowCount > 0 && <AlertTriangle className="w-3 h-3 text-yellow-400 shrink-0" />}
                No-shows
              </div>
            </div>
            <div className="text-center">
              <div className={`text-2xl font-black ${completionRate === null ? "text-muted-foreground" : completionRate >= 90 ? "text-green-400" : completionRate >= 70 ? "text-yellow-400" : "text-red-400"}`}>
          ...
          {noShowCount === 0 && (
            <div className="mt-2 text-xs text-green-500 font-bold text-center">✓ Never abandoned a request</div>
          )}
          {noShowCount > 0 && (
            <p className="mt-2 text-[10px] text-muted-foreground leading-relaxed">
              A no-show is recorded when a helper claims a request then cancels before completing it. Life happens — context matters.
            </p>
          )}
        </div>

        {/* Languages */}
        {helper.helper_languages && helper.helper_languages.length > 0 && (
          <div className="bg-card border border-border rounded-2xl p-4">
            <div className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3 flex items-center gap-1.5">
              <Languages className="w-3.5 h-3.5" /> Languages
            </div>
            <div className="flex flex-wrap gap-2">
              {helper.helper_languages.map(lang => (
                <span key={lang} className="text-xs bg-primary/10 border border-primary/30 text-primary px-3 py-1.5 rounded-full font-bold">
                  {lang}
                </span>
              ))}
            </div>
        ...
        {helper.helper_bio && (
          <div className="bg-card border border-border rounded-2xl p-4">
            <div className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-2">About</div>
            <p className="text-sm text-foreground leading-relaxed">{helper.helper_bio}</p>
          </div>
        )}
