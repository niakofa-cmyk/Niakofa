...
const CATEGORIES = [
  ...
  { value: "paperwork",         label: "📄 Paperwork Help" },
...
function MatchedHelpersSection({ lat, lng }: { lat: number | null; lng: number | null }) {
  ...
  // hasLoadedRef: spinner only shown on first load — background re-fetches (lat/lng
  // change) run silently so existing helper cards don't disappear momentarily.
  ...
  useEffect(() => {
    ...
    fetch(`${base}/api/recurring/matched-helpers?${params}`, {
      ...
      .catch(() => {
        // Keep whatever helpers were visible — a blip shouldn't blank the section.
        if (!hasLoadedRef.current) setMessage("Could not load matched helpers");
      })
      .finally(() => setLoading(false));
  }, [lat, lng]);

  // Only show spinner when we've never successfully loaded (no cached data to show).
...
export default function RecurringScreen() {
  ...
  // hasLoadedRef: spinner only on first load — re-fetches (e.g. after toggle/delete)
  // run silently so the card list doesn't disappear between mutations.
  ...
  const load = useCallback(async () => {
    ...
    // Re-fetches (after create/delete/toggle) run silently so the list stays visible.
    if (!subscriptionsLoadedRef.current) setLoading(true);
    try {
      ...
      // On a network blip, keep existing subscriptions visible rather than flashing empty.
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="px-4 py-4 pb-24 space-y-6 max-w-lg mx-auto">
        {loading && !subscriptionsLoadedRef.current ? (
