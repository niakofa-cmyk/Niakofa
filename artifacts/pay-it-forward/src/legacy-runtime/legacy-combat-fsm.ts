...
export class LegacyCombatController {
  ...
  lightAttack(targets: LegacyCombatTarget[]) {
    ...
    const anim: LegacyCombatAnimState = this.comboStage === 1 ? "lightAttack2" : "lightAttack1";
    this.comboExpiresAtMs = this.nowMs + COMBO_WINDOW_MS;
    this.playCombatAnim(anim, () => this.resolveHit(targets, this.comboStage === 1 ? 12 : 8));
  }
  ...
  private resolveHit(targets: LegacyCombatTarget[], damage: number, popUp?: { dy: number }) {
  ...
  private playCombatAnim(anim: LegacyCombatAnimState, onComplete?: () => void) {
    // Bridges into the base actor's frame-advance logic; a real integration
    // would extend LegacyActorController.state.anim to accept
    // LegacyFullAnimState instead of duplicating the timer here.
    (this.actor as unknown as { playAction: (a: any, cb?: () => void) => void }).playAction(
      anim,
      onComplete
    );
  }
