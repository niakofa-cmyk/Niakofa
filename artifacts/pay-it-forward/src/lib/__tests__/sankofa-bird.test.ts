...
// ─────────────────────────────────────────────────────────────────────────────
// 12. Gaze vector — Phase 12 real-time directional awareness
// ─────────────────────────────────────────────────────────────────────────────
describe("computeGazeVector", () => {
  // Silence TS unused-import warning — GazeDir8 is used as type annotation below
  const _gd: GazeDir8 = "center";
  void _gd;

  it("no inputs → center gaze, both axes zero", () => {
    const { gazeDirX, gazeDirY, gazeDir8 } = computeGazeVector({
      bankDeg: 0, upcomingTurn: null, landingPhase: "flying",
      speedMs: 5,
    });
    expect(gazeDir8).toBe("center");
    expect(Math.abs(gazeDirX)).toBeLessThan(0.28); // below MAJOR threshold
    expect(Math.abs(gazeDirY)).toBeLessThan(0.28);
  });

  it("upcoming-turn=left → look left (negative X, gazeDir8=left)", () => {
    const { gazeDirX, gazeDir8 } = computeGazeVector({
      bankDeg: 0, upcomingTurn: "left", landingPhase: "flying",
      speedMs: 8,
    });
    expect(gazeDirX).toBeLessThan(-0.28);
    expect(gazeDir8).toMatch(/left/);
  });

  it("upcoming-turn=right → look right (positive X, gazeDir8=right)", () => {
    const { gazeDirX, gazeDir8 } = computeGazeVector({
      bankDeg: 0, upcomingTurn: "right", landingPhase: "flying",
      speedMs: 8,
    });
    expect(gazeDirX).toBeGreaterThan(0.28);
    expect(gazeDir8).toMatch(/right/);
  });

  it("takeoff phase → look up (negative Y)", () => {
    const { gazeDirY, gazeDir8 } = computeGazeVector({
      bankDeg: 0, upcomingTurn: null, landingPhase: "takeoff",
      speedMs: 0,
    });
    expect(gazeDirY).toBeLessThan(-0.28);
    expect(gazeDir8).toMatch(/up/);
  });

  it("hover phase → look down (positive Y)", () => {
    const { gazeDirY, gazeDir8 } = computeGazeVector({
      bankDeg: 0, upcomingTurn: null, landingPhase: "hover",
      speedMs: 0,
    });
    expect(gazeDirY).toBeGreaterThan(0.28);
    expect(gazeDir8).toMatch(/down/);
  });

  it("approaching flag → look down (positive Y)", () => {
    const { gazeDirY, gazeDir8 } = computeGazeVector({
      bankDeg: 0, upcomingTurn: null, landingPhase: "flying",
      speedMs: 5, approaching: true,
    });
    expect(gazeDirY).toBeGreaterThan(0.28);
    expect(gazeDir8).toMatch(/down/);
  });

  it("bankDeg right + upcoming-turn right → strong right gaze", () => {
    const { gazeDirX, gazeDir8 } = computeGazeVector({
      bankDeg: 18, upcomingTurn: "right", landingPhase: "flying",
      speedMs: 10,
    });
    // Both bank (positive X) and upcoming turn (positive X) push right
    expect(gazeDirX).toBeGreaterThan(0.5);
    expect(gazeDir8).toMatch(/right/);
  });

  it("gazeDirX is clamped to [-1, 1]", () => {
    // Extreme bank + anticipatory turn could theoretically exceed ±1 without clamping
    const { gazeDirX } = computeGazeVector({
      bankDeg: 25, upcomingTurn: "right", landingPhase: "flying",
      speedMs: 100,
    });
    expect(gazeDirX).toBeLessThanOrEqual(1);
    expect(gazeDirX).toBeGreaterThanOrEqual(-1);
  });

  it("gazeDirY is clamped to [-1, 1]", () => {
    const { gazeDirY } = computeGazeVector({
      bankDeg: 0, upcomingTurn: null, landingPhase: "hover",
      speedMs: 0, approaching: true,
    });
    expect(gazeDirY).toBeLessThanOrEqual(1);
    expect(gazeDirY).toBeGreaterThanOrEqual(-1);
  });

  it("diagonal: upcoming-turn=left + takeoff → up-left", () => {
    const { gazeDir8, gazeDirX, gazeDirY } = computeGazeVector({
      bankDeg: 0, upcomingTurn: "left", landingPhase: "takeoff",
      speedMs: 0,
    });
    // Both axes active → diagonal
    expect(gazeDirX).toBeLessThan(-0.18);
    expect(gazeDirY).toBeLessThan(-0.18);
    expect(gazeDir8).toBe("up-left");
  });

  it("idle phase → center Y gaze", () => {
    const { gazeDirY } = computeGazeVector({
      bankDeg: 0, upcomingTurn: null, landingPhase: "idle",
      speedMs: 0,
    });
    expect(gazeDirY).toBe(0);
  });

  it("returns a valid GazeDir8 string for all phase/turn combinations", () => {
    const validDirs = new Set<GazeDir8>([
      "center","left","right","up","down","up-left","up-right","down-left","down-right"
    ]);
    const phases: LandingPhase[] = ["flying","idle","hover","perch","slowflap","dive","takeoff"];
    const turns = [null, "left", "right"] as const;
    for (const phase of phases) {
      for (const turn of turns) {
        const { gazeDir8 } = computeGazeVector({
          bankDeg: 0, upcomingTurn: turn, landingPhase: phase, speedMs: 5,
        });
        expect(validDirs.has(gazeDir8)).toBe(true);
      }
    }
  });
});
