...
export const sankofaCssBase = `
        ...
        /* --diagonal-pose-intensity: 0–1 scalar driving the back-diagonal body
           skew in phase-21.ts. Registered so CSS transitions can interpolate
           it smoothly (especially inside calc() for skewX). Without this
           declaration Safari 15.4 cannot interpolate the var() inside the
           skewX() call and the back-diagonal snap is instant rather than eased. */
        @property --diagonal-pose-intensity {
          syntax: '<number>';
          inherits: false;
          initial-value: 0;
        }
