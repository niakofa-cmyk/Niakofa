...
export function SankofaBirdSvg({
  ...
  // Phase cycles 0-7 — covers all 8 compass gaze directions for omnidirectional drift.
  ...
  // ── Phase 13: Aerodynamic mode ────────────────────────────────────────────────
  // Derived from speed, landing phase, and explicit prop overrides.
  // Sets data-aero-mode on the rig so CSS can gate P13 effects by mode
  // without re-evaluating the priority logic in CSS selectors.
  ...
  // bankDeg is passed so the bird glances toward the turn direction even when
  // no nav upcoming-turn signal exists (e.g. on the map screen without route data).
  const gazeDir: GazeDirection = computeGazeVector({
    ...
    bankDeg: navigating ? bankDeg : undefined,
  });
