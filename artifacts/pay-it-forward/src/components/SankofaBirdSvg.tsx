...
export interface SankofaBirdProps {
   ...
   * Body pitches ~28° forward, wings pump rapidly, legs stride hard.
   * Head stays horizon-locked (stabilized) against the body pitch.
   * Observed in young/ground-dwelling birds climbing steep slopes.
   */
  wairMode?: boolean;
  /**
   * Phase 13: Dynamic soaring mode (albatross-style).
   * Bird locks wings near-horizontal and alternates dive-into-headwind with
   * climb-in-tailwind, extracting energy from wind-speed gradients.
   * 4.2 s cycle: nose-down dive → low sweep → climbing pull → arc top.
   */
  soaring?: boolean;
  /**
   * Phase 13: Courtship/mating display.
   * Lateral body pivot, exaggerated wing fans, head bow sequence, tail spread,
   * crown fully erect + color-flash. Based on Red-capped Manakin sliding +
   * Sandhill Crane bowing motifs. 1.6 s looping cycle.
   */
...
export function SankofaBirdSvg({
  ...
  // ── Phase 12: Auto-saccade (idle micro-drift) ─────────────────────────────
  ...
  // ── Phase 12: Saccade cycling — subtle idle gaze drift every 3-6 s ─────────
  useEffect(() => {
    if (navigating || celebrating || newNotification) return;
    const delay = 3000 + Math.random() * 3000;
    const id = setTimeout(() => setSaccadePhase(p => nextSaccadePhase(p)), delay);
  ...
  // ── Phase 12: Real-time gaze direction ────────────────────────────────────────
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute transition-transform duration-150 ease-linear sankofa-bird-container"
        style={{
          width: size,
          height: size,
        ...
        >
          ...
          >
            ...
            <g className="sankofa-bird-legs">
              ...
              {/* P13.6: Knee joint highlights — tibiotarsus bend point.
                  Visible at mid+ zoom (CSS rules control opacity).
                  Left knee sits at the mid-point of the left leg line (18.5,29.5 → 16.5,34). */}
              <circle
      ...
      <style>{`
        ...
        .sankofa-bird-rig[data-flying="true"][data-speed="running"]:not([data-battery-saver="true"]):not([data-wair="true"]):not([data-soaring="true"]) .sankofa-bird-wing-right {
          animation: sankofa-figure8-right calc(var(--flap-period, 450ms) + 18ms) ease-in-out infinite !important;
        }
        ...
        .sankofa-bird-rig[data-flying="true"][data-speed="driving"]:not([data-battery-saver="true"]):not([data-wair="true"]):not([data-soaring="true"]) .sankofa-bird-wing-right {
          animation: sankofa-figure8-right calc(var(--flap-period, 200ms) + 18ms) ease-in-out infinite !important;
        }
        ...
        .sankofa-bird-rig[data-wair="true"]:not([data-battery-saver="true"]) .sankofa-bird-wing-right {
          animation: sankofa-wair-wing-right calc(0.22s + 18ms) linear infinite !important;
        }
        ...
        .sankofa-bird-rig[data-mating="true"]:not([data-battery-saver="true"]) .sankofa-bird-wing-right {
          animation: sankofa-mating-wing-right calc(1.6s + 18ms) ease-in-out infinite !important;
        }
        ...
        .sankofa-bird-rig[data-flying="true"][data-speed="walking"]:not([data-battery-saver="true"]) .sankofa-leg-right {
          animation: sankofa-knee-bob-right 0.9s ease-in-out infinite !important;
        ...
        .sankofa-bird-rig[data-flying="true"][data-speed="running"]:not([data-battery-saver="true"]) .sankofa-leg-right {
          animation: sankofa-knee-bob-right 0.45s ease-in-out infinite !important;
        ...
        @keyframes sankofa-murmur-wave {
          ...
          28%   { filter: hue-rotate(18deg)  brightness(1.18); }
          52%   { filter: hue-rotate(32deg)  brightness(1.0); }
          76%   { filter: hue-rotate(12deg)  brightness(1.10); }
