/**
 * LegacyJournalPanel — the player's real, recorded journal.
 *
 * Extracted from the old standalone /legacy/journal page so the exact same
 * component can be:
 *   1. mounted embedded, as a slide-over inside the live chapter runtime
 *      (LegacyChapterPlay), so opening the journal never leaves the world, and
 *   2. mounted as the body of the /legacy/journal route, for deep links and
 *      sharing.
 *
 ...
 * or the player's own recorded choice. Nothing here is AI-narrated or
 * fabricated; see artifacts/api-server/src/routes/legacy-chapters.ts
...
export interface LegacyJournalPanelProps {
  /** Family whose journal to load. Required — the panel does no family resolution itself. */
  familyId: number | null;
  /** Called when the player dismisses the panel. In embedded mode this closes
   *  the overlay; in standalone mode the page wrapper passes navigate("/legacy"). */
  onClose: () => void;
  /** When true, jump straight to a given chapter's revisit action instead of navigating a route.
   *  Embedded callers (the live chapter runtime) can use this to jump scenes in place. */
  onRevisitChapter?: (chapterId: number) => void;
  /** Embedded rendering drops the sticky full-page header chrome (used inside an overlay sheet
   *  that already provides its own header) and fills its container instead of the viewport. */
  embedded?: boolean;
}

export function LegacyJournalPanel({ familyId, onClose, onRevisitChapter, embedded = false }: LegacyJournalPanelProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/legacy/journal/${familyId}`, { headers: authHeaders() });
        if (!res.ok) {
  ...
  const revisit = (chapterId: number) => {
    if (onRevisitChapter) onRevisitChapter(chapterId);
    else onClose();
  };

  return (
    <div className={embedded ? "flex flex-col h-full bg-[#0e1111] text-stone-100" : "min-h-[100dvh] bg-[#0e1111] text-stone-100 pb-28"}>
      {/* Header */}
      <div className={embedded
        ? "sticky top-0 z-20 bg-[#0e1111]/95 backdrop-blur border-b border-stone-800/50 flex-shrink-0"
        : "sticky top-0 z-20 bg-[#0e1111]/95 backdrop-blur border-b border-stone-800/50"}>
        <div className="max-w-lg mx-auto px-4 py-3 flex items-center gap-3">
          <button onClick={onClose} className="p-2 -ml-2 rounded-lg hover:bg-stone-800/50 transition-colors">
            {embedded ? <X className="w-5 h-5 text-stone-400" /> : <ArrowLeft className="w-5 h-5 text-stone-400" />}
          </button>
      ...
      <div className={embedded ? "max-w-lg mx-auto px-4 py-6 overflow-y-auto flex-1" : "max-w-lg mx-auto px-4 py-6"}>
        {loading && (
        ...
        {!loading && !error && entries.length === 0 && (
          <div className="text-center py-16">
            ...
            >
              {embedded ? "Back to the Story" : "Begin a Journey"}
            </button>
          </div>
        )}
          ...
          <>
            ...
            {filteredGroups.map((group) => (
              <div key={group.chapterId} className="mb-8">
                ...
                <button
                  onClick={() => revisit(group.chapterId)}
