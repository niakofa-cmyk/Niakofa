...
const requireApproved = jest.fn((_req: unknown, _res: unknown, next: unknown) => next());
...
beforeEach(() => {
  jest.clearAllMocks();
  requireApproved.mockImplementation((_req: unknown, _res: unknown, next: unknown) => next());
