...
router.post("/audio-circle-sessions/:id/heartbeat", requireAuth, generalApiLimiter, async (req, res) => {
  ...
  // The client is untrusted: only broadcast IDs belonging to an active
  // participant in this same session. Otherwise a client could make an
  // unrelated user appear to be speaking in the room.
