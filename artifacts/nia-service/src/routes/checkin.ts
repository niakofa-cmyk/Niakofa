...
interface CheckinPayload {
  ...
  sessionId: string;
  // Set by api-server's nia-checkin-worker when the requester's diaspora
  // hub (via users.community_id -> diaspora_hubs.community_id) is currently
  // flagged is_crisis. Previously this endpoint had no way to know that at
  // all, so a resident of a hub in active crisis still got the same generic
  // "how did it go" prompt as everyone else.
  hubInCrisis?: boolean;
}
...
router.post("/checkin", verifyInternalSecret, async (req: Request, res: Response) => {
  ...
  const { userId, requestId, requestTitle, category, helperName, sessionId, hubInCrisis } = payload;
  if (
