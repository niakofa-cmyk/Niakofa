...
import { db, stripeAccountsTable, paymentTransactionsTable, usersTable, requestsTable, transactionsTable, communityPoolLedgerTable, walletCashoutsTable, diasporaHubsTable, diasporaHubPledgesTable, civicInvoicesTable } from "@workspace/db";
  ...
  try {
    switch (event.type) {
      case "payment_intent.succeeded": {
        ...
        // 0c. Civic invoice payment? Mark the invoice as paid.
        if (pi.metadata?.["civic_invoice_id"]) {
          const invoiceId = parseInt(pi.metadata["civic_invoice_id"]);
          const userId = parseInt(pi.metadata["user_id"] ?? "0") || null;
          const [paid] = await db
            .update(civicInvoicesTable)
            .set({ status: "paid", paid_at: new Date(), paid_by_user_id: userId })
            .where(and(
            ...
            .returning();
          if (paid) {
            logger.info({ invoice_id: invoiceId, pi: pi.id, amount: pi.amount / 100 }, "civic-invoice: payment succeeded, invoice marked paid");
          }
          break;
        }
