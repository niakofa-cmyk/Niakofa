...
**How to apply:** on a fresh Replit dev DB with an empty `users` table, run `pnpm --filter @workspace/scripts run seed-test-accounts -- --i-know-this-is-production` (defaults are fine for dev). Default admin login: `admin@niakofa.app` / `NiakofaAdmin2026!` (override via `SEED_ADMIN_PASSWORD` if needed).
