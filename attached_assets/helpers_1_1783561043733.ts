...
router.get("/helpers/online", requireAuth, async (req, res) => {
  ...
  // Geo-filtering: prefer an indexed PostGIS ST_DWithin query against the
  // geog column (see migration 0052) — this pushes the radius filter and
  // the distance calc into the DB instead of pulling a bounding box of rows
  // into Node and Haversine-scanning them. Falls back to the old
  // bounding-box + JS-Haversine approach only if PostGIS/geog isn't
  // available, same pattern as /requests/nearby in requests.ts.
  let helpersWithDistance: ((typeof usersTable.$inferSelect) & { distance_miles: number | null })[];
  ...
  if (location) {
    try {
      ...
      `);
      helpersWithDistance = nearbyRows.rows as typeof helpersWithDistance;
    } catch (geoErr) {
      logger.warn({ err: geoErr }, "helpers/online: PostGIS unavailable, falling back to bounding-box + Haversine");
      ...
      const boundingBoxHelpers = await query.where(and(...conditions));
      helpersWithDistance = boundingBoxHelpers
        .filter(h => h.lat !== null && h.lng !== null)
        .map(h => ({ ...h, distance_miles: distanceMiles(location.lat, location.lng, h.lat!, h.lng!) }))
        .filter(h => h.distance_miles <= radius);
    }
  } else {
    const query = db.select().from(usersTable).$dynamic();
    const noLocationHelpers = await query.where(
      and(eq(usersTable.helper_mode_active, true), inArray(usersTable.id, optedInIdSet))
    );
    helpersWithDistance = noLocationHelpers
      .filter(h => h.lat !== null && h.lng !== null)
      .map(h => ({ ...h, distance_miles: null }));
  }

  const result = helpersWithDistance
    .map(h => {
      const dist = h.distance_miles;
