...
export default function MapScreen() {
  ...
  const zoomIn = useCallback(() => { mapRef.current?.zoomIn({ duration: 200 }); haptic("light"); }, []);
  const zoomOut = useCallback(() => { mapRef.current?.zoomOut({ duration: 200 }); haptic("light"); }, []);
  ...
  return (
    <div className="relative w-full h-[100dvh] overflow-hidden bg-background">
      ...
      {/* Single status slot — see mapStatus computation above. Positioned
          ...
          jumps around or competes for space. The live-stats overlay is now
          a single collapsed pill (not a 5-pill stack), so right-20 gives
          this banner a bit more width on narrow phones than the old
          right-24 did while still clearing the pill. */}
      {mapStatus && (
        <div className="absolute top-20 left-4 right-20 z-10 flex justify-center" style={{ zIndex: Z_CHROME }}>
      ...
      <div className="absolute top-20 right-4 flex flex-col items-end gap-1.5" style={{ zIndex: Z_CHROME }}>
        <button
          type="button"
      ...
      {!mapError && viewMode === "map" && (
        <MapControlsPanel
          ...
          controlsRecede={sheetExpanded && helperModeActive && openRequests.length > 0}
        />
      )}
