 ...
 */
export function getHubLeadershipTrustBonus(
  approvedAt: Date | string | null | undefined,
  isApproved: boolean,
): number {
  if (!isApproved || approvedAt == null) return 0;

  const approved = approvedAt instanceof Date ? approvedAt : new Date(approvedAt);
  if (isNaN(approved.getTime())) return 0;

  const monthsLeading = (Date.now() - approved.getTime()) / (1000 * 60 * 60 * 24 * 30.44);

  if (monthsLeading >= 12) return 6;
  if (monthsLeading >= 6)  return 4;
  if (monthsLeading >= 3)  return 2;
  return 0;
}
