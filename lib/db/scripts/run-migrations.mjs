...
async function main() {
  ...
  // Railway databases use TLS by default. CI's disposable PostgreSQL service
  // is local and intentionally non-TLS, so allow that environment to opt out
  // explicitly without weakening the production default.
  const ssl = /^(0|false|no|disable)$/i.test(process.env.DATABASE_SSL ?? "")
    ? false
