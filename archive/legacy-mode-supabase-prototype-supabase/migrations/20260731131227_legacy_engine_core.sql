/*
# Niakofa Legacy Engine — Core Schema

This migration creates the full Legacy Engine domain: family vault tables,
legacy worlds, chapters, scenes, dialogues, choices, consequences, sessions,
quests, achievements, artifacts, and knowledge versioning.
...
- `legacy_consequences` — outcomes of choices
...
- `legacy_artifacts` — collectible items with real provenance
- `legacy_world_versions` — knowledge version snapshots
- `family_knowledge_versions` — versioned family knowledge hashes

## Security
- RLS enabled on all tables
- Policies scoped to `anon, authenticated` (no-auth app model for prototype)
...
-- Provides a demo family so the Legacy Mode is explorable immediately.
